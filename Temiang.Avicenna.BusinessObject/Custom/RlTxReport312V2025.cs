...
{
    public partial class RlTxReport312V2025
    {
        ...
        public static void Process(int fromMonth, int toMonth, int year, string surgerySpecialty, out int khusus, out int besar, out int sedang, out int kecil, out int lain)
        {
            ...
            episodeq.Where(episodeq.IsVoid == false);
            episodeq.Where(episodeq.IsFromOperatingRoom == true);
            episodeq.Where(regq.IsVoid == false);
            episodeq.Where(bookingq.SmfID.In(parValueList));
            episodeq.Where(bookingq.SmfID.IsNotNull() & bookingq.SmfID != "");
