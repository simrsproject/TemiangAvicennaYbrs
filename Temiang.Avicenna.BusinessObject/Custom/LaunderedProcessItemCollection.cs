...
{
    public partial class LaunderedProcessItemCollection
    {
        public DataTable ItemOutstandingWithReceivedDate(DateTime receiveDateFrom, DateTime receiveDateTo, string serviceUnitId)
        {
            ...
            string commandText =
                ...
                FROM [LaundryReceivedItem] a WITH (NOLOCK) 
                INNER JOIN [LaundryReceived] b WITH (NOLOCK) ON a.[ReceivedNo] = b.[ReceivedNo] 
                LEFT JOIN 
                (
	                SELECT xx.ProcessNo, xx.ProcessSeqNo, xx.ReceivedNo, xx.ReceivedSeqNo, xx.Qty 
	                FROM [LaunderedProcessItem] xx WITH (NOLOCK) 
	                INNER JOIN [LaunderedProcess] yy WITH (NOLOCK) ON yy.ProcessNo = xx.ProcessNo AND yy.IsVoid = 0
                ) c ON c.ReceivedNo = b.ReceivedNo AND c.ReceivedSeqNo = a.ReceivedSeqNo
                INNER JOIN [Item] e WITH (NOLOCK) ON a.[ItemID] = e.[ItemID] 
                INNER JOIN [AppStandardReferenceItem] f WITH (NOLOCK) ON (a.[SRItemUnit] = f.[ItemID] AND f.[StandardReferenceID] = 'ItemUnit') 
                INNER JOIN [ServiceUnit] g WITH (NOLOCK) ON b.[FromServiceUnitID] = g.[ServiceUnitID] 
                LEFT JOIN [ServiceRoom] h WITH (NOLOCK) ON b.[FromRoomID] = h.[RoomID] 
        ...
        public DataTable GetQtyProcessed(string processNo, string processSeqNo, string receivedNo, string receivedSeqNo)
        {
            ...
            string commandText =
                ...
                FROM [LaunderedProcessItem] x WITH (NOLOCK) 
                INNER JOIN [LaunderedProcess] y WITH (NOLOCK) ON y.ProcessNo = x.ProcessNo AND y.IsVoid = 0 ";
