namespace Temiang.Avicenna.ReportLibrary.RLib_Slip.BillingCharges.Charges.ARSANI
{
    ...
    public partial class PaymentReceiveDetail : Telerik.Reporting.Report
    {
            ...
            if (hd.LoadByPrimaryKey(printJobParameters[0].ValueString))
            {
                if (hd.IsApproved == true)
                {
                    ...
                    details.Query.Where(
                        ...
                        details.Query.SRPaymentType == Temiang.Avicenna.Common.AppSession.Parameter.PaymentTypeCorporateAR
                        );
                    ...
                    if (grr.GuarantorID == Temiang.Avicenna.Common.AppSession.Parameter.SelfGuarantor)
                        txtTotalP.Value = string.Format("{0:n2}", table.AsEnumerable().Sum(t => t.Field<decimal>("PatientAmount")) + Convert.ToDecimal(txtAdministrationAmount.Value));
                    else
                    {
                        var gtor = new Guarantor();
                        gtor.LoadByPrimaryKey(reg.GuarantorID);
                        if (gtor.IsIncludeAdminValue ?? false)
                            txtTotalG.Value = string.Format("{0:n2}", table.AsEnumerable().Sum(t => t.Field<decimal>("GuarantorAmount")) + Convert.ToDecimal(txtAdministrationAmount.Value));
                        else
                            txtTotalP.Value = string.Format("{0:n2}", table.AsEnumerable().Sum(t => t.Field<decimal>("PatientAmount")) + Convert.ToDecimal(txtAdministrationAmount.Value));
                    }
