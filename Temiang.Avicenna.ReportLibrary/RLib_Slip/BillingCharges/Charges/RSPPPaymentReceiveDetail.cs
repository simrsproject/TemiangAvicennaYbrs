...
namespace Temiang.Avicenna.ReportLibrary.RLib_Slip.BillingCharges.Charges
{
    ...
    public partial class RSPPPaymentReceiveDetail : Report
    {
        public RSPPPaymentReceiveDetail(string programID, PrintJobParameterCollection printJobParameters)
        {
            /// <summary>
            ...
            if (hd.LoadByPrimaryKey(printJobParameters[0].ValueString))
            {
                if (hd.IsApproved == true)
                {
                    ...
                    txtAdministrationAmount.Value = string.Format("{0:n0}", regis.AdministrationAmount);
                    ...
                    txtTotal.Value = string.Format("{0:n0}", Convert.ToDecimal(table.AsEnumerable().Select(t => t.Field<Decimal>("Total")).Sum() + regis.AdministrationAmount));
