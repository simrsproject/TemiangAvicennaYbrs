...

{
    ...
    /// </summary>
    public partial class PaymentReceiveReceiptOPNoLogo : Telerik.Reporting.Report
    {
        public PaymentReceiveReceiptOPNoLogo(string programID, PrintJobParameterCollection printJobParameters)
        {
            /// <summary>
            ...
            textBox30.Value = string.Format("Rp. {0:n0}", (total));
            txtTotalAmountInWords.Value = (new Common.Convertion()).NumericToWords(total);
            //DataSource = dtb;

            var dtbsp = new ReportDataSource().GetDataTable(programID, printJobParameters);
            this.DataSource = dtbsp;
