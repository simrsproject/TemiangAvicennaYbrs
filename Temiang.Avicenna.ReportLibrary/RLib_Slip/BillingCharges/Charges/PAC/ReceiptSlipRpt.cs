...
namespace Temiang.Avicenna.ReportLibrary.RLib_Slip.BillingCharges.Charges.PAC
{
    ...
    /// </summary>
    public partial class ReceiptSlipRpt : Report
    {
        public ReceiptSlipRpt(string programID, PrintJobParameterCollection printJobParameters)
        {
            /// <summary>
            ...
            {   
                ...
                {
                    txtStatus.Value = (hd.PrintNumber > 1 ? "(D)" : "");
                    ...
                    cost.Select
                        ...
                        // header
                            ...
                            reg.PlavonAmount,
                            @"<HealthcareName = (SELECT HealthcareName FROM Healthcare WHERE HealthcareID = '001') >",
                           ...
                           // @"<Total = (l.ChargeQuantity * l.[Price]) - l.[DiscountAmount]>",
                    ...
                    cost.Where(charges.PackageReferenceNo.IsNull(), charges.IsApproved == true);
                    cost.OrderBy
                        (
                    ...
                    var tab = cost.LoadDataTable();
                    if (tab.AsEnumerable().Any())
                        table.Merge(tab.AsEnumerable().Where(c => c.Field<decimal>("ChargeQuantity") != 0).CopyToDataTable());
                        ...
                        if (row["IsPrintWithDoctorName"].ToString() == "1")
                        {
                            ...
                            if (comp != null && !string.IsNullOrEmpty(comp.ParamedicID))
                            {
                                ...
                                if (doc.LoadByPrimaryKey(comp.ParamedicID))
                                    row["ItemName"] = row["ItemName"] +" - "+ doc.ParamedicName;
                            }
                        }
                    }
                    ...
                    //string transSeq = table.AsEnumerable().Select(t => (t.Field<string>("TransactionNo") + t.Field<string>("SequenceNo")))
                    //                                      .Aggregate(string.Empty, (current, transNo) => current + (transNo + "','"));
                    ...
                    txtTotalAmountInWords.Value = (new Common.ConvertionToEnglish()).NumericToWords(Convert.ToDecimal(tbl.Rows[0]["SumTotal"]));
