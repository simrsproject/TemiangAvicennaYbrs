...
namespace Temiang.Avicenna.ReportLibrary.RLib_Slip.BillingCharges.Billing
{
    ...
    /// </summary>
    public partial class BillingIntermStatementPatientIncComponent : Telerik.Reporting.Report
    {
        public BillingIntermStatementPatientIncComponent(string programID, PrintJobParameterCollection printJobParameters)
        {
            //
            ...
            if (regis.DischargeDate.HasValue)
            {
                textBox123.Value = string.Format("{0:dd-MMM-yyyy HH:mm}", regis.DischargeDate.HasValue ?
                    ...
                    DateTime.Now);
                var dd = ((regis.DischargeDate ?? DateTime.Now.Date) - dateEntry.Date).Days;
                if (dd == 0) dd++;
            ...
            else
            {
                if (srRegType == "IPR" && regis.AllowPatientCheckOutDateTime.HasValue)
                {
                    ...
                    var dd = ((regis.AllowPatientCheckOutDateTime ?? DateTime.Now.Date) - dateEntry.Date).Days;
                    if (dd == 0) dd++;
                    else if (AppSession.Parameter.IsBillingStatementLosCalculationWithAdd1Day) dd++;
