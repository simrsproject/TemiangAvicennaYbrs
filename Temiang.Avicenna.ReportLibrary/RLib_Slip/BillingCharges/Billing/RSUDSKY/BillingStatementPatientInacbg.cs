...
namespace Temiang.Avicenna.ReportLibrary.RLib_Slip.BillingCharges.Billing.RSUDSKY
{
    ...
    /// <summary>
    /// Summary description for BillingStatementPatientInacbg.
    /// </summary>
    public partial class BillingStatementPatientInacbg : Telerik.Reporting.Report
    {
        public BillingStatementPatientInacbg(string programID, PrintJobParameterCollection printJobParameters)
        {
            //
            ...
            textBox22.Value = printJobParameters.FindByParameterName("UserName").ValueString;

            var Director = new AppParameter();
            Director.LoadByPrimaryKey("Director");
            textBox25.Value = "(" + Director.ParameterValue + ")";
            ...
            {
                textBox54.Value = (guar.GuarantorHeaderID == AppSession.Parameter.SelfGuarantor) ? "PRIBADI" : "PERUSAHAAN / ASURANSI";
            ...
            textBox123.Value = string.Format("{0:dd-MMM-yyyy}", regis.DischargeDate.HasValue ? regis.DischargeDate.Value : DateTime.Now);
            ...
            // footer
            var cov = new RegistrationCoverageDetailCollection();
            cov.Query.Where(cov.Query.RegistrationNo == regis.RegistrationNo);
            cov.LoadAll();
            decimal selisih = 0;
            decimal kelasperawatan = 0;
            
            foreach (var c in cov)
            {
                selisih += c.CalculatedAmount ?? 0;
                if (c.ClassID == regis.ChargeClassID)
                    kelasperawatan = c.CoverageAmount ?? 0;
            }

            if (selisih == 0)
                kelasperawatan = regis.PlavonAmount ?? 0;

            textBox72.Value = string.Format("{0:N2}", kelasperawatan); //kelas perawatan
            textBox31.Value = string.Format("{0:N2}", regis.PlavonAmount); //hak kelas
            textBox34.Value = string.Format("{0:N2}", selisih); //selisih
