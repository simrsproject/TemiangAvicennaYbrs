...
namespace Temiang.Avicenna.ReportLibrary.RLib_Slip.BillingCharges.Billing.RSCH.GDBaru
{
    ...
    /// </summary>
    public partial class BillingIntermStatementPatientIncComponentRekap : Telerik.Reporting.Report
    {
        public BillingIntermStatementPatientIncComponentRekap(string programID, PrintJobParameterCollection printJobParameters)
        {
            //
            ...
            var ar = new TransPaymentQuery("a");
            var arDet = new TransPaymentItemQuery("b");

            ar.Select
                (arDet.Amount, ar.PrintReceiptAsName
                );
            ar.InnerJoin(arDet).On(ar.PaymentNo == arDet.PaymentNo);
            ar.Where(arDet.SRPaymentType == "PaymentType-004");
            ar.Where(ar.RegistrationNo == printJobParameters.FindByParameterName("RegNo").ValueString);
            ar.Where(ar.IsApproved == true);
            ar.Where(ar.IsVoid == false);

            DataTable dtb = ar.LoadDataTable();
            ...
            foreach (DataRow row in dtb.Rows)
            {
                ...
                {
                    notes += "    - " + Convert.ToString(row["PrintReceiptAsName"]) + " = " + string.Format("Rp. {0:n0}", (Convert.ToDecimal(row["Amount"]))) +"." + Environment.NewLine;
            ...
            if (dtb.Rows.Count > 1)
            {
                textBox147.Value = "Plafond " + string.Format("Rp. {0:n0}", (total)) + " terdiri dari : " + Environment.NewLine + notes;
