...
namespace Temiang.Avicenna.ReportLibrary.RLib_Slip.BillingCharges.Billing
{
    ...
    public partial class BillingIntermStatementPatientIncComponentByDate : Telerik.Reporting.Report
    {
        public BillingIntermStatementPatientIncComponentByDate(string programID, PrintJobParameterCollection printJobParameters)
        {
            ...
            textBox48.Value = string.Format("{0:dd-MMM-yyyy HH:mm}", dateEntry);
            if (regis.DischargeDate.HasValue)
            {
                textBox123.Value = string.Format("{0:dd-MMM-yyyy HH:mm}", regis.DischargeDate.HasValue ?
                    ...
                    DateTime.Now);
                var dds = ((regis.DischargeDate ?? DateTime.Now) - dateEntry.Date).Days;
                if (dds == 0) dds++;
                else if (AppSession.Parameter.IsBillingStatementLosCalculationWithAdd1Day) dds++;

                textBox159.Value = string.Format("{0} hari", dds.ToString());
            ...
            else
            {
                if (srRegType == "IPR" && regis.AllowPatientCheckOutDateTime.HasValue)
                {
                    ...
                    var dds = ((regis.AllowPatientCheckOutDateTime ?? DateTime.Now) - dateEntry.Date).Days;
                    if (dds == 0) dds++;
                    else if (AppSession.Parameter.IsBillingStatementLosCalculationWithAdd1Day) dds++;
                ...
                else
                {
                    if (srRegType == "IPR" && regis.DischargePlanDate.HasValue)
                    {
                        ...
                        var dds = ((regis.DischargePlanDate ?? DateTime.Now) - dateEntry.Date).Days;
                        if (dds == 0) dds++;
                        else if (AppSession.Parameter.IsBillingStatementLosCalculationWithAdd1Day) dds++;
            ...
            if (hc.LoadByPrimaryKey("HealthcareInitialAppsVersion") && hc.ParameterValue == "RSTJ")
            {
