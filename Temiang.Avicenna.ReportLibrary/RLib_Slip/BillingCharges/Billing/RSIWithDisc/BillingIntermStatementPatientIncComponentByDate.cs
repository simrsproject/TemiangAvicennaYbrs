...
namespace Temiang.Avicenna.ReportLibrary.RLib_Slip.BillingCharges.Billing.RSIWithDisc
{
    ...
    /// <summary>
    /// Summary description for BillingIntermStatementPatientIncComponent.
    /// </summary>
    public partial class BillingIntermStatementPatientIncComponentByDate : Telerik.Reporting.Report
    {
        public BillingIntermStatementPatientIncComponentByDate(string programID, PrintJobParameterCollection printJobParameters)
        {
            //
            ...
            Helper.InitializeLogoOnlyLeft(this.pageHeaderSection1);
            ...
            if (AppSession.Parameter.IsBillingStatementRegDateUsingCheckinConfirmed)
            {
            ...
            textBox157.Visible = textBox158.Visible = textBox159.Visible = (srRegType == "IPR");
            ...
            textBox162.Value = p.StreetName + ' ' + p.City;
            ...
            // kelas ruang
            textBox4.Value = srRegType == "IPR" ? "Kelas / No.Tempat Tidur" : "Poliklinik";
            ...
            if (guar.LoadByPrimaryKey(regis.GuarantorID))
            {
                ...
                if (guarhd.LoadByPrimaryKey(guar.GuarantorHeaderID))
                {
                    ...
                    AppStandardReferenceItem grType = new AppStandardReferenceItem();
                    if (grType.LoadByPrimaryKey("GuarantorType", guarhd.SRGuarantorType))
                    {
                        textBox54.Value = grType.ItemName;
                    }
                }
            }
            ...
            if (regis.DischargeDate.HasValue)
            {
                textBox123.Value = string.Format("{0:dd-MMM-yyyy HH:mm}", regis.DischargeDate.HasValue ?
                    ...
                    DateTime.Now);
                var dd = ((regis.DischargeDate ?? DateTime.Now) - dateEntry.Date).Days;
                if (dd == 0) dd++;
                else if (AppSession.Parameter.IsBillingStatementLosCalculationWithAdd1Day) dd++;

                textBox159.Value = string.Format("{0} hari", dd.ToString());
            ...
            else
            {
                if (srRegType == "IPR" && regis.AllowPatientCheckOutDateTime.HasValue)
                {
                    ...
                    var dd = ((regis.AllowPatientCheckOutDateTime ?? DateTime.Now) - dateEntry.Date).Days;
                    if (dd == 0) dd++;
                    else if (AppSession.Parameter.IsBillingStatementLosCalculationWithAdd1Day) dd++;
                ...
                else
                {
                    if (srRegType == "IPR" && regis.DischargePlanDate.HasValue)
                    {
                        ...
                        var dd = ((regis.DischargePlanDate ?? DateTime.Now) - dateEntry.Date).Days;
                        if (dd == 0) dd++;
                        else if (AppSession.Parameter.IsBillingStatementLosCalculationWithAdd1Day) dd++;
            ...
            //pictureBox1.Visible = (srRegType == "IPR");

            textBox13.Value += srRegType == "IPR" ? " PERAWATAN" : (" PEMERIKSAAN " + svu.ServiceUnitName);
            ...
            textBox139.Value = srRegType == "IPR" ? "Email: " + healthcare.EmailAddr : "Telp. " + healthcare.PhoneNo + " Fax " + healthcare.FaxNo;

            var hc = new AppParameter();
            if (hc.LoadByPrimaryKey("HealthcareInitialAppsV.RSIWithDiscon") && hc.ParameterValue == "RSTJ")
            {
                textBox131.Visible = false;
                textBox38.Value = "Petugas";
                textBox39.Value = printJobParameters.FindByParameterName("UserName").ValueString;
            }

            var au = new AppUser();
            au.LoadByPrimaryKey(printJobParameters.FindByParameterName("UserID").ValueString);
            pictureBox1.Value = au.SignatureImage != null ? Convert.ToBase64String(au.SignatureImage) : null;
