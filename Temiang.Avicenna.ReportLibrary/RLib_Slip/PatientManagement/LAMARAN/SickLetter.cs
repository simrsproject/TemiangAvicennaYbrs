...
namespace Temiang.Avicenna.ReportLibrary.RLib_Slip.PatientManagement.LAMARAN
{
    public partial class SickLetter : Report
    {
        public SickLetter(string programID, PrintJobParameterCollection printJobParameters)
        {
            ...
            query.Select(par.ParamedicName, pat.PatientName,reg.AgeInYear, occ.ItemName.As("Occupation"), query.StartDate, query.EndDate,
                @"<DATEDIFF(dd, a.StartDate, a.EndDate) + 1 AS 'Days'>",
                 "<CASE WHEN d.sex = 'F' THEN 'P' ELSE 'L' END AS 'Sex'>",
                query.Notes);
