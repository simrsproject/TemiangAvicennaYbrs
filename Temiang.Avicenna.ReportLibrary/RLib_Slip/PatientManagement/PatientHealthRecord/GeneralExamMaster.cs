...
{
    class GeneralExamMaster
    {
        public static DataTable GetDataSourceV2(string TransactionNo, string RegistrationNo,
        ...
        //public static void detail_ItemDataBound(object sender, EventArgs e)
        //{

        //}

        public static void SetObjectSource(Telerik.Reporting.Processing.DetailSection section,
        Telerik.Reporting.Processing.Table crosstbl, Telerik.Reporting.Processing.Table tbl,
        ref List<int> CTColWidth, Telerik.Reporting.Processing.PictureBox pb1)
        {
            ...
            switch (SRAnswerType)
            {
                ...
                case "SIG":
                    {
                        try
                        {
                            var img = ByteArrayToImage((byte[])section.DataObject["BodyImage"]);

                            if (SRAnswerType == "SIG")
                                img = ResizeImage(img, new Size(100, 80), true, InterpolationMode.Default);

                            pb1.Image = img;

                            var w = (SRAnswerType == "SIG" ? (new Unit(100)) : (new Unit(img.Size.Width))).ChangeType(UnitType.Mm);
                            var h = (SRAnswerType == "SIG" ? (new Unit(80)) : (new Unit(img.Size.Height))).ChangeType(UnitType.Mm);
                            var l = (new Unit(0.1, UnitType.Inch)).ChangeType(UnitType.Mm);
                            var maxw = (section.Report.Width.ChangeType(UnitType.Mm)) - (l);
                            if (w > maxw)
                            {
                                // calculate new prop size
                                var nw = maxw;
                                var nh = nw / w * h;
                                w = nw; h = nh;
                            }

                            pb1.Size = new SizeU(w, h);
                            pb1.Location = new PointU(new Unit(0.1, UnitType.Inch), new Unit(0, UnitType.Inch));
                ...
                default:
                    {
                        SetTableSource(section, crosstbl, tbl, ref CTColWidth);
                        break;
                    }
            }
        }

        private static Image ResizeImage(Image image, Size size, bool preserveAspectRatio, InterpolationMode interpolationMode)
        {
            int newWidth;
            int newHeight;
        ...
        #region RSMP
        public static void FillCheckBox(PatientHealthRecordLineCollection pColl, Telerik.Reporting.TextBox TextBox00, Telerik.Reporting.TextBox TextBox01, string QuestionID)
        {
            try
            {
                var q = pColl.Where(x => QuestionID.Contains(x.QuestionID)).FirstOrDefault();
                //TextBox00.Value = q.QuestionText;
                FillCheckBox(TextBox01, q.QuestionAnswerText.Equals("1"));
            }
            catch
            {
                TextBox00.Value = "ERR";
                FillCheckBox(TextBox01, false);
            }
        }
