    ...
    public partial class PatientBirthRecordRpt : Telerik.Reporting.Report
    {
        public PatientBirthRecordRpt(string programID, PrintJobParameterCollection printJobParameters)
        {
           ...
           // Helper.InitializeLogo(this.pageHeaderSection1);
            ...
            DateTime? tgllahirAyah;
            if (tbl.Rows.Count > 0)
            {
                tgllahirAyah = Convert.ToDateTime(tbl.Rows[0]["FatherBirthDate"]);
                textBox92.Value = Common.Helper.GetAgeInYear(tgllahirAyah ?? DateTime.Now.Date).ToString();
            }
