namespace Temiang.Avicenna.ReportLibrary.Accounting.BalanceSheet
{
    ...
    /// </summary>
    public partial class BalanceSheetComparationReportPerPeriode : Telerik.Reporting.Report
    {
        private ReportDataSource reportDataSource  = new ReportDataSource();

        public BalanceSheetComparationReportPerPeriode(string programID, PrintJobParameterCollection printJobParameters)
        {
            /// <summary>
        ...
        {
            string yearStart = printJobParameters.FindByParameterName("pYMSMEPostingPeriode_YearStart").ValueString;
            string yearEnd = printJobParameters.FindByParameterName("pYMSMEPostingPeriode_YearEnd").ValueString;
            string month = printJobParameters.FindByParameterName("pYMSMEPostingPeriode_Month").ValueString;

            this.txtFinalBalance1.Value = string.Format("{1} - {0}", yearStart, Utility.MonthName(month));
            this.txtFinalBalance2.Value = string.Format("{1} - {0}", yearEnd, Utility.MonthName(month));

            this.txtPeriode.Value = string.Format("Periode: {0}", month);
