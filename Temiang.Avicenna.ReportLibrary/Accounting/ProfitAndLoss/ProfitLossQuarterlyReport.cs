namespace Temiang.Avicenna.ReportLibrary.Accounting.ProfitLoss
{
    ...
    /// </summary>
    public partial class ProfitLossQuarterlyReport : Telerik.Reporting.Report
    {
        private ReportDataSource reportDataSource = new ReportDataSource();

        public ProfitLossQuarterlyReport(string programID, PrintJobParameterCollection printJobParameters)
        {
            /// <summary>
        ...
        {
            string q = printJobParameters.FindByParameterName("pSingleQuarterPeriode_Quarter").ValueString;
            string year = printJobParameters.FindByParameterName("pSingleQuarterPeriode_Year").ValueString;

            int querterINT = Convert.ToInt32(q);
            string quarter = string.Format("Quarter {0}", querterINT);

            this.txtPeriode.Value = string.Format("Periode: {0} - {1}", quarter, year);
