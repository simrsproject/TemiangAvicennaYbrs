    ...
    public partial class AwalRawatJalanAnak : Telerik.Reporting.Report
    {
        ...
        private void PopulateBirtHIstory(string patientID)
        {
            ...
            if (pbr.LoadByPrimaryKey(patientID))
            {
                ...
                txtasi.Value = Convert.ToString(pbr.AsiToMonthAge);
                txtDiet.Value = pbr.CurrentDiet;
                txtTengkurap.Value = Convert.ToString(pbr.ProneAtMonthAge);
                txtDuduk.Value = Convert.ToString(pbr.SitAtMonthAge);
                txtMerangkak.Value = Convert.ToString(pbr.CrawlAtMonthAge);
                txtBerdiri.Value = Convert.ToString(pbr.StandUpAtMonthAge);
                txtBerjalan.Value = Convert.ToString(pbr.WalkAtMonthAge);
                txtBicara36.Value = Convert.ToString(pbr.Speak3WordAtMonthAge);
                txtBicaraKalimat.Value = Convert.ToString(pbr.Speak2SentAtMonthAge);
