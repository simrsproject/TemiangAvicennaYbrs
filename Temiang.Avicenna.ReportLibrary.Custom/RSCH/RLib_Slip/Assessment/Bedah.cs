...
namespace Temiang.Avicenna.ReportLibrary.RSCH.RLib_Slip.Assessment
{
    ...
    /// </summary>
    public partial class Bedah : Telerik.Reporting.Report
    {
        public Bedah(string programID, PrintJobParameterCollection printJobParameters)
        {
            //
        ...
        private void PopulateLocalistStatus(PatientAssessment asses)
        {
            ...
            if (loc.LoadAll())
            {
                ...
                {
                    picLocalistStatus01.Value = imgHelper.ConvertByteArrayToImage(loc[0].BodyImage);
                }

                if (loc.Count > 1 && loc[1] != null)
                {
                    picLocalistStatus02.Value = imgHelper.ConvertByteArrayToImage(loc[1].BodyImage);
                }
            }
        }
