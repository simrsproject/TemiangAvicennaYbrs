...
namespace Temiang.Avicenna.ReportLibrary.RSMP.RLib_Slip.Assessment
{
    ...
    public partial class Gigi : Report
    {
        ...
        public Gigi(string programID, PrintJobParameterCollection printJobParameters)
        {
            ...
            // RM.05.a	ASESMEN AWAL RAWAT JALAN POLI GIGI
            var nphrColl = new PatientHealthRecordCollection();
            var qrLastPhr = new PatientHealthRecordQuery("a");
            qrLastPhr.Where(qrLastPhr.RegistrationNo == reg.RegistrationNo, qrLastPhr.QuestionFormID == "RM.05.a");
            qrLastPhr.es.Top = 1;
            ...
            PopulateOdontogram(patientID, reg.RegistrationNo);
    ...
    txtHeight);
        ...
        private DataTable DentalStatus()
        {
            var stdi = new AppStandardReferenceItemQuery();
            stdi.Where(stdi.StandardReferenceID == "DentalStatus");
        ...
        {
            var dentalStatus = DentalStatus();
            var sb = new StringBuilder();
            foreach (DataRow row in dentalStatus.Rows)
            {
                sb.AppendFormat("{0}: {1}", row["ItemID"], row["ItemName"]);
                sb.AppendLine(string.Empty);
            }
            txtDentalStatus.Value = sb.ToString();
