...
}else{this._disposeHandler(d,"mousemove","_mouseMoveDelegate");
this._disposeHandler(d,"mouseout","_mouseOutDelegate");
}}},_disposeHandler:function(g,d,f){var e=this[f];
if(e){$removeHandler(g,d,e);
this[f]=null;
}},_mouseMove:function(d){if(this._zoomRatio==null){this._viewPort=this._imageEditor.get_viewport();
}this._zoomRatio=100/this._imageEditor.getEditableImage().get_zoomLevel();
...
},_downloadImg:function(j){var h=j.location;
var f=j.name;
var g=j.value;
...
f.set_text($telerik.htmlDecode(f.get_text()));
...
this._downloadImg({location:g,name:"base64",value:this.getBase64()});
...
if(h.indexOf("Telerik.Web.UI.WebResource.axd?type=iec")!=-1){this._downloadImg({location:h+e+"1",name:"encryptedDownloadKey",value:this._getEncryptedDownloadKey()});
}else{var g=document.location+"";
g=g.substring(0,g.lastIndexOf("/")+1);
g=g+"Telerik.Web.UI.WebResource.axd?type=iec"+e+this._getCustomDownloadKey();
this._downloadImg({location:g,name:"encryptedDownloadKey",value:this._getEncryptedDownloadKey()});
}}},_getCustomDownloadKey:function(){return"custom";
},_getEncryptedDownloadKey:function(){return this._imageEditor._downloadKey;
...
},dispose:function(){this._disposeToolsPanel();
this._disposeToolBarPanel();
this._disposeToolWidget();
...
},_disposeToolsPanel:function(){this._disposeDock(this.get_toolsPanel());
},_disposeToolBarPanel:function(){var c=this.get_toolBarPanel();
if(c&&!c.get_dockZoneID()){this._disposeDock(c);
}},_disposeToolWidget:function(){if(this._currentToolWidget){this._currentToolWidget.dispose();
this._currentToolWidget=null;
}},_disposeStatusBar:function(){if(this._statusBar){this._statusBar.dispose();
}},_disposeDock:function(c){var e=this.get_element();
if(!e||!c){return;
}var d=c.get_element();
if(d){e.appendChild(d);
}c.dispose();
},_disposeHandler:function(f,c,e){var d=this[e];
if(d){$removeHandler(f,c,d);
this[e]=null;
...
var e=this.get_toolsPanel();
e.get_contentContainer().style.minHeight="52px";
var d=this._renderMode===Telerik.Web.UI.RenderMode.Lite?"rdContent":"rdMiddle";
$telerik.getElementByClassName(e.get_element(),d).id=this.get_id()+"_rdMiddle";
c.show(this.get_id()+"_rdMiddle");
}},openToolsPanel:function(){var c=this.get_toolsPanel();
if(!this._isDialogMoved||(c.get_top()==0&&c.get_left()==0)){var h=this.get_location(true);
...
this.updateClientState();
this._repaintToolbarControlIfWider();
}if(this._width=="100%"){this.get_element().style.width="auto";
}},_repaintToolbarControlIfWider:function(){var d=this._getToolbarControl(0);
if(d&&d.repaint){var c=d.get_element();
if(c.scrollWidth>c.offsetWidth){d.repaint();
...
if(c){var d=this.get_toolsLoadPanelType();
...
}else{if(this._imageLoadedDelegate){this._editableImage.remove_imageReload(this._imageLoadedDelegate);
delete this._imageLoadedDelegate;
...
if(this._renderMode===Telerik.Web.UI.RenderMode.Classic){if(c==180){d.set_width(270);
}else{if(c==220){d.set_width(360);
}else{if(c==200){d.set_width(310);
}}}}else{this.get_toolsPanel().get_element().style.width="auto";
...
if(!this._toolbarResizeHandler){this._initializeToolbarResizeHandler();
}this.raiseEvent("resizeStart",Sys.EventArgs.Empty);
},_initializeToolbarResizeHandler:function(){var c=this.get_toolGroups();
this._toolbarResizeHandler=a.throttle(250,false,function(){for(var d=0;
d<c.length;
d++){c[d].repaint();
}});
...
},onResizing:function(c){this._toolbarResizeHandler();
var d=this.get_viewport();
...
}else{this._disposeHandler(this.get_element(),"keydown","_keyDownDelegate");
...
Type.registerNamespace("Telerik.Web.UI.ImageEditor");
(function(a,c,b,d){b.DraggableResizeBox=function(e,g){var f=g||{};
this.set_element(e);
this._offsetElement=f.offsetElement;
...
h._widgetScroller=new b.WidgetScroller(h._resizable);
this._widgetScroller.add_scrolling(a.proxy(this._onScrolling,this));
...
h._resizeEndDelegate=Function.createDelegate(h,h._resizableOnResizeEnd);
g.add_resizeEnd(h._resizeEndDelegate);
},_onScrolling:function(f,e){if(this._resizeDir){this._recalculateScrollWidgetDelta();
}else{this._draggable.updateScroll(e.delta);
}this._raiseResizeEvent();
},_recalculateScrollWidgetDelta:function(){var e=this.getBounds();
e.dir=this._resizeDir;
this._manageScroller(e);
},_resizableOnResizeStart:function(f,e){this._offset=this.getPosition();
this._size=this.getSize();
this._widgetScroller.startScrolling();
},_resizableOnResizeEnd:function(f,e){this._resizeDir=null;
this._widgetScroller.stopScrolling();
...
k.dir=h;
this._resizeDir=h;
this._manageScroller(k);
p._raiseResizeEvent(k);
},_manageScroller:function(e){var g=this._widgetScroller;
var f=this._resizable;
g.manageScrolling(e);
if(g._scrollerX||g._scrollerY){f.__startX=f._startX;
f.__startY=f._startY;
}else{if(g._scrollTimer&&g._scrollTimer.get_enabled()){f._startX=f.__startX;
f._startY=f.__startY;
...
var e=h._boxElement;
var g={offsetElement:this._offsetElement||h.get_element(),ownerDocument:e.ownerDocument};
var f=h._draggable=new c.Widgets.Draggable(e,g);
h._dragStartDelegate=Function.createDelegate(h,h._draggableOnDragStart);
f.add_dragStart(h._dragStartDelegate);
h._draggingDelegate=Function.createDelegate(h,h._draggableOnDragging);
f.add_dragging(h._draggingDelegate);
h._dragEndDelegate=Function.createDelegate(h,h._draggableOnDragEnd);
f.add_dragEnd(h._dragEndDelegate);
},_draggableOnDragStart:function(f,e){this._widgetScroller.startScrolling();
},_draggableOnDragging:function(j,e){var f=j._element;
var h=e.get_newPosition();
var i=this.getBounds();
var g=new c.Bounds(h.x,h.y,f.offsetWidth,f.offsetHeight);
i.dir=g.dir=this._getDraggingDirection(e);
this._manageScroller(i);
this._raiseResizeEvent(g);
},_getDraggingDirection:function(e){var f="";
if(e._delta.x>0){f+="E";
}if(e._delta.x<0){f+="W";
}if(e._delta.y>0){f+="S";
}if(e._delta.y<0){f+="N";
}return f;
},_draggableOnDragEnd:function(f,e){this._widgetScroller.stopScrolling();
...
e._resizable.remove_resizeEnd(e._resizeEndDelegate);
...
if(e._draggable){e._draggable.remove_dragStart(e._dragStartDelegate);
e._draggable.remove_dragging(e._draggingDelegate);
e._draggable.remove_dragEnd(e._dragEndDelegate);
...
},set_constraints:function(e){this._constraints=e;
var f=this._draggable;
if(f){f.set_constraints({minX:e.x,minY:e.y,maxX:e.width,maxY:e.height});
...
b.DraggableImageResizeBox=function(e,f){this._imageElement=null;
this._jImageElement=null;
this.IsImageBroken=false;
b.DraggableImageResizeBox.initializeBase(this,[e,f]);
};
...
b.DraggableTextBox.prototype={dispose:function(){this._disposeDraggableWidget();
this._boxContent.remove();
this._boxElement.remove();
this._boxElement=this._boxContent=null;
},_disposeDraggableWidget:function(){var f=this;
var e=f._draggable;
if(e){e.remove_dragStart(f._dragStartDelegate);
e.remove_dragging(f._draggingDelegate);
e.remove_dragEnd(f._dragEndDelegate);
e.dispose();
...
this._widgetScroller=new b.WidgetScroller(e);
f._dragStartDelegate=Function.createDelegate(f,f._draggableOnDragStart);
e.add_dragStart(f._dragStartDelegate);
f._draggingDelegate=Function.createDelegate(f,f._draggableOnDragging);
e.add_dragging(f._draggingDelegate);
f._dragEndDelegate=Function.createDelegate(f,f._draggableOnDragEnd);
e.add_dragEnd(f._dragEndDelegate);
},_draggableOnDragStart:function(f,e){this._widgetScroller.startScrolling();
},_draggableOnDragging:function(i,e){var g=i._element;
var h=e.get_newPosition();
var f=new c.Bounds(h.x,h.y,g.offsetWidth,g.offsetHeight);
f.dir=this._getDraggingDirection(e);
this._widgetScroller.manageScrolling(f);
a(this).trigger("boundsChanged",a.extendEventArgs(new Sys.EventArgs(),{position:{x:h.x,y:h.y}}));
},_getDraggingDirection:function(e){var f="";
if(e._delta.x>0){f+="E";
}if(e._delta.x<0){f+="W";
}if(e._delta.y>0){f+="S";
...
(function(a,c,b){Type.registerNamespace("Telerik.Web.UI.ImageEditor");
b.WidgetScroller=function(e,d){this._resizeExtender=e;
this.element=this._resizeExtender._element;
this.setOptions(d);
};
b.WidgetScroller.prototype={get_constraints:function(){return this.options.constraints;
},set_constraints:function(d){this.options.constraints=d;
},setOptions:function(d){this.options=a.extend({scrollEdgeConstSmall:1,scrollEdgeConst:2,scrollSpeed:3,constraints:null},d||{});
...
this._scrollTimer.set_interval(15);
},manageScrolling:function(d){var e=a(this.element);
this.options.bounds=d;
var f=e.parent();
this._scrollerX=this._calculateScrollDelta(d.x,e.width(),f.width(),f.scrollLeft(),d.dir);
this._scrollerY=this._calculateScrollDelta(d.y,e.height(),f.height(),f.scrollTop(),d.dir);
this._initScrolling();
},stopScrolling:function(){delete this.options.bounds;
if(this._scrollTimer){this._scrollTimer.dispose();
}},add_scrolling:function(d){a(this).bind("scrolling",d);
},remove_scrolling:function(d){a(this).unbind("scrolling",d);
},raiseEvent:function(e,d){a(this).trigger(e,[d]);
},_scrollViewport:function(){var d=a(this.element);
var f=d.parent();
var g=this._getScrollPosition(f);
var i=g.left+this._scrollerX;
var j=g.top+this._scrollerY;
f.scrollLeft(i);
f.scrollTop(j);
var e=this._getScrollPosition(f);
var h=new c.Point(e.left-g.left,e.top-g.top);
this.raiseEvent("scrolling",{oldScroll:g,newScroll:e,delta:h});
},_constrainPosition:function(f){var d=this.get_constraints();
if(d){var e=this.element;
f.x=Math.max(Math.min(f.x,d.horizontal-e.outerWidth()),0);
f.y=Math.max(Math.min(f.y,d.vertical-e.outerHeight()),0);
}return f;
},_initScrolling:function(){if(this._scrollTimer){var d=this._scrollerX!=0||this._scrollerY!=0;
this._scrollTimer.set_enabled(d);
}},_calculateScrollDelta:function(i,j,k,d,e){var f=false;
var g=false;
if(e){f=e.indexOf("N")>-1;
g=e.indexOf("W")>-1;
}var h=f||g;
if(h&&i<d+this.options.scrollEdgeConstSmall){return 0-this.options.scrollSpeed;
}else{if(i+j+this.options.scrollEdgeConst>d+k){return this.options.scrollSpeed;
}else{return 0;
...
b.WidgetScroller.registerClass("Telerik.Web.UI.ImageEditor.WidgetScroller");
})($telerik.$,Telerik.Web.UI,Telerik.Web.UI.ImageEditor);
