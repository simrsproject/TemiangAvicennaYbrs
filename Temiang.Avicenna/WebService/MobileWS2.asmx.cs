...
{
    /// <summary>
    /// Summary description for MobileWS2
    /// </summary>
    ...
    public class MobileWS2 : V1_1.RegistrationWS
    {
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void VitalSignByMedicalNo(string AccessKey, string MedicalNo, string VitalSignType, string RecordCount)
        {
            var log = LogAdd();
            try
            {
                ...
                ValidateAccessKey(AccessKey);
                InspectStringRequired("RecordCount", RecordCount);
                ...
                if (pat.LoadByMedicalNo(MedicalNo))
                {
                    ...
                    if (regs.Length > 0)
                    {
                        ...
                        if (quests.Length > 0)
                        {
                            ...
                            {
                                query.es.Top = RecordCount.ToInt();
                            }
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void PatientRegistrationHistory(string AccessKey, string MedicalNo, string ServiceUnitID, string ParamedicID, string DateStart, string DateEnd)
        {
            var log = LogAdd();
            try
            {
                ...
                {
                    r.Where(
                        r.RegistrationDate.Between(DateStart, DateEnd)
                        );
                }
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void PatientLabResultList(string AccessKey, string MedicalNo, int RecordCount) {
            var log = LogAdd();
            try
            {
                //Validating AccessKey
                ...
                InspectStringRequired("RecordCount", RecordCount.ToString());

                var tc = new TransChargesQuery("b");
                var toUnit = new ServiceUnitQuery("tu");
                tc.InnerJoin(toUnit).On(tc.ToServiceUnitID == toUnit.ServiceUnitID);

                var fromUnit = new ServiceUnitQuery("fu");
                tc.InnerJoin(fromUnit).On(tc.FromServiceUnitID == fromUnit.ServiceUnitID);
                ...
                tc.Where(
                    ...
                    tc.IsApproved == true);
                tc.OrderBy(tc.ExecutionDate.Descending, tc.TransactionNo.Descending);

                if (RecordCount > 0) {
                    tc.es.Top = RecordCount;
                }
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void PatientLabResultDetail(string AccessKey, string TransactionNo)
        {
            var log = LogAdd();
            try
            {
                //Validating AccessKey
                ValidateAccessKey(AccessKey);
                InspectStringRequired(TransChargesMetadata.ColumnNames.TransactionNo, TransactionNo);
                ...
                if (tc.LoadByPrimaryKey(TransactionNo))
                {
                    ...
                    {
                        case "LINK_LIS":
                            var labNo = ""; // <-- ini nomor apa ya?
                            dtb = Temiang.Avicenna.Module.RADT.Emr.MainContent.ExamOrderHistCtl
                                .LaboratoryResult(tc.RegistrationNo, TransactionNo, labNo);
                            break;
                        default:
                            dtb = Temiang.Avicenna.Module.RADT.Emr.MainContent.ExamOrderHistCtl
                                .LaboratoryResult(tc.RegistrationNo, TransactionNo);
                            break;
                    }
                    ...
                    throw new Exception(ErrDataNotFound.Replace(GetErrorMessage(ErrDataNotFound),
                        string.Format("Transaction number {0} not found", TransactionNo)));
                }
            }
            catch (Exception ex)
            {
        ...
        public void AppointmentGetListByParamedicIDAppointmentDate(string AccessKey, string ParamedicID, string AppointmentDate)
        {
            ...
            try
            {
                ...
                appt.LeftJoin(pat).On(appt.PatientID == pat.PatientID)
                    ...
                    .InnerJoin(asri).On(asri.StandardReferenceID == "AppointmentStatus" && appt.SRAppointmentStatus == asri.ItemID)
                    .Select(appt.AppointmentNo, appt.AppointmentDate, appt.AppointmentTime, asri.ItemName.As("AppointmentStatus"),
                    ...
                    .Where(appt.ParamedicID == ParamedicID, appt.AppointmentDate == dAppointmentDate, appt.SRAppointmentStatus != AppParameter.GetParameterValue(AppParameter.ParameterItem.AppointmentStatusCancel))
                    .OrderBy(appt.AppointmentQue.Ascending);
        ...
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void RegistrationGetListIpByParamedicID(string AccessKey, string ParamedicID,
        string GuarantorID, string ClassID, string RoomID, string EwsStatus, string RecordCount)
        {
            var log = LogAdd();
            try
            {
                ...
                reg.InnerJoin(pat).On(reg.PatientID == pat.PatientID)
                    ...
                    .InnerJoin(cl).On(reg.ClassID == cl.ClassID)
                    .InnerJoin(parteam).On(reg.RegistrationNo == parteam.RegistrationNo)
                    .Select(reg.PatientID, reg.RegistrationNo, reg.FromRegistrationNo, pat.MedicalNo, pat.PatientName, pat.DateOfBirth, reg.AgeInYear, reg.AgeInMonth, reg.AgeInDay, pat.Sex,
                        guar.GuarantorName, sr.RoomName, cl.ClassName
                    ).Where(reg.SRRegistrationType == AppConstant.RegistrationType.InPatient,
                ...
                var objColl = tbl.AsEnumerable().Select(x => new
                {
                    ...
                    Age = string.Format("{0}th {1}bl {2}hr", x["AgeInYear"], x["AgeInMonth"], x["AgeInDay"]).ToString(),
                    Sex = x["Sex"].ToString(),
                    GuarantorName = x["GuarantorName"].ToString(),
                    RoomName = x["RoomName"].ToString(),
                    ClassName = x["ClassName"].ToString(),
            ...
            try
            {
                ...
                pg.InnerJoin(pgd).On(pg.PaymentGroupNo == pgd.PaymentGroupNo)
                    ...
                    .Where(pgd.ParamedicID == ParamedicID,
                        ...
                        fee.ParamedicID == ParamedicID, pg.PaymentDate.Between(dStart, dEnd)
                    ).Select(
                        pg.PaymentGroupNo,
                        pg.PaymentDate,
                        fee.RegistrationNo,
        ...
        public void RegistrationSummaryByParamedicID(string AccessKey, string ParamedicID)
        {
            ...
            try
            {
                ...
                var gr = new GuarantorQuery("g");
                reg.InnerJoin(pat).On(reg.PatientID == pat.PatientID);
                reg.LeftJoin(unit).On(reg.ServiceUnitID == unit.ServiceUnitID);
                reg.InnerJoin(gr).On(reg.GuarantorID == gr.GuarantorID);
                reg.Where(reg.SRRegistrationType != AppConstant.RegistrationType.InPatient,
                    reg.IsVoid == false, reg.IsNonPatient == false, reg.IsFromDispensary == false, reg.ParamedicID == ParamedicID,
                    "<reg.RegistrationDate = CAST(getdate() as date)>"
                    ).Select("<'Rawat Jalan' JenisRawat>", reg.RegistrationNo, pat.MedicalNo, pat.PatientName, unit.ServiceUnitName, gr.GuarantorName);
                var tbl = reg.LoadDataTable();

                reg = new RegistrationQuery("reg");
                pat = new PatientQuery("pat");
                unit = new ServiceUnitQuery("su");
                gr = new GuarantorQuery("g");
                reg.InnerJoin(pat).On(reg.PatientID == pat.PatientID);
                reg.LeftJoin(unit).On(reg.ServiceUnitID == unit.ServiceUnitID);
                reg.InnerJoin(gr).On(reg.GuarantorID == gr.GuarantorID);
                reg.Where(reg.SRRegistrationType == AppConstant.RegistrationType.InPatient,
                    ...
                    ).Select("<'Rawat Inap' JenisRawat>", reg.RegistrationNo, pat.MedicalNo, pat.PatientName, unit.ServiceUnitName, gr.GuarantorName);
                var tbl2 = reg.LoadDataTable();

                var data = tbl.AsEnumerable().Union(tbl2.AsEnumerable());

                var result = data.GroupBy(row => row.Field<string>("JenisRawat"))
                                        .Select(group => new
                                        {
                                            Name = group.Key,
                                            Value = group.Count(),
                                            Detail = group.Select(row => new
                                            {
                                                ...
                                                ServiceUnit = row.Field<string>("ServiceUnitName"),
                                                Guarantor = row.Field<string>("GuarantorName")
                                            }).ToArray()
                                        });
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void PrescriptionList(string AccessKey, string MedicalNo, string RecordCount)
        {
            var log = LogAdd();
            try
            {
                ...
                var query = new TransPrescriptionQuery("tp");
                ...
                query.InnerJoin(regq).On(query.RegistrationNo == regq.RegistrationNo)
                    ...
                    .InnerJoin(parq).On(query.ParamedicID == parq.ParamedicID);
                query.Select(
                ...
                query.Where(patq.MedicalNo == MedicalNo);
                query.OrderBy(query.PrescriptionNo.Descending, query.PrescriptionDate.Descending);
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void PrescriptionDetail(string AccessKey,string PrescriptionNo)
        {
            var log = LogAdd();
            try
            {
                ValidateAccessKey(AccessKey);
                InspectStringRequired(TransPrescriptionItemMetadata.ColumnNames.PrescriptionNo, PrescriptionNo);

                var query = new TransPrescriptionItemQuery("tpi");
                var it = new ItemQuery("it");
                var cm = new ConsumeMethodQuery("cm");

                query.InnerJoin(it).On(query.ItemID == it.ItemID)
                    .InnerJoin(cm).On(query.SRConsumeMethod == cm.SRConsumeMethod);
                query.Select(
                    it.ItemID,
                    it.ItemName,
                    query.PrescriptionQty,
                    cm.SRConsumeMethodName.As("ConsumeMethod")
                    );
