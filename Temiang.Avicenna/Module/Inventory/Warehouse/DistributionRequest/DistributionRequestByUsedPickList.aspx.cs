...
{
    public partial class DistributionRequestByUsedPickList : BasePageDialog
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(Page.Request.QueryString["igid"]))
            {
                ...
                ig.Where(ig.ItemGroupID == Page.Request.QueryString["igid"]);

                cboItemGroupID.DataSource = ig.LoadDataTable();
                cboItemGroupID.DataBind();
                cboItemGroupID.SelectedValue = Page.Request.QueryString["igid"];
            }
        }
        ...
        //private DataTable LoadRequestItemSelectionDataTable2()
        //{
        ...
        //    tci.Where(tc.IsBillProceed == true, 
        //        tc.Or(tc.IsPackage.IsNull(), tc.IsPackage == false),
        //        tc.Or(tc.PackageReferenceNo.IsNull(), tc.PackageReferenceNo == false),
        //        tc.LastUpdateDateTime
        //        );
        ...
        private DataTable LoadRequestItemSelectionDataTable()
        {
            ...
            im.Where(im.MovementDate >= txtFromDate.SelectedDate.Value.Date, im.MovementDate <= txtToDate.SelectedDate.Value.Date, 
                im.LocationID == Request.QueryString["lid"]);
            ...
            query.LeftJoin(qrBalanceWh).On(qrBalanceWh.LocationID == Request.QueryString["tlid"] && qrBalanceWh.ItemID == query.ItemID);
            query.Select(@"<ISNULL(cwb.Balance, 0) AS BalanceCw>");
        ...
        private DataTable InitializeDataFromSalesHist()
        {
            ...
            sales.Where(
                sales.LocationID == Request.QueryString["lid"]
                );
            ...
            var qrBalanceWh = new ItemBalanceQuery("cwb");
            sales.LeftJoin(qrBalanceWh).On(qrBalanceWh.LocationID == Request.QueryString["tlid"] && qrBalanceWh.ItemID == sales.ItemID);
            sales.Select(@"<ISNULL(cwb.Balance, 0) AS BalanceCw>");
