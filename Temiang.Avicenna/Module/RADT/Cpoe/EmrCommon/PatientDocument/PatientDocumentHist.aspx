<%@ Page Language="C#" MasterPageFile="~/MasterPage/MasterDialog.Master" AutoEventWireup="true"
    CodeBehind="PatientDocumentHist.aspx.cs" Inherits="Temiang.Avicenna.Module.RADT.Emr.PatientDocumentHist" %>
    ...
    <telerik:RadCodeBlock ID="radCodeBlock" runat="server">
        ...
        <script type="text/javascript" language="javascript">
            function tbarMain_OnClientButtonClicking(sender, args) {
                ...
                switch (val) {
                    ...
                    case "newupload":
                        ...
                        break;
                    case "batchupload":
                        var url = '<%= Helper.UrlRoot() %>/Module/RADT/Cpoe/EmrCommon/PatientDocument/PatientDocumentBatchUpload.aspx?progid=<%= ProgramID %>&patid=<%= PatientID %>&regno=<%= RegistrationNo %>&ccm=rebind&cet=<%=grdDocument.ClientID %>';
                        openWinMaximize(url);
                        break;
                    case "newwebcam":
                        var url = '<%= Helper.UrlRoot() %>/Module/RADT/Cpoe/EmrCommon/PatientDocument/PatientDocumentWebCam.aspx?mod=new&progid=<%= ProgramID %>&patid=<%= PatientID %>&regno=<%= RegistrationNo %>&ccm=rebind&cet=<%=grdDocument.ClientID %>&surgno=<%= Request.QueryString["surgno"] %>';
            ...
            function showDetail(mode, pdid) {
                var url = '<%= Helper.UrlRoot() %>/Module/RADT/Cpoe/EmrCommon/PatientDocument/PatientDocumentUpload.aspx?mod=' + mode + '&progid=<%= ProgramID %>&pdid=' + pdid +'&patid=<%= PatientID %>&regno=<%= RegistrationNo %>&ccm=rebind&cet=<%=grdDocument.ClientID %>&surgno=<%= Request.QueryString["surgno"] %>';
            ...
            function ShowFile(id, fileName) {
                if (fileName.toLowerCase().includes(".dcm") == true) // DICOM file gunakan external app
                {
                    ...
                    if (fileName.toLowerCase().includes(".pdf") == true) {
                        ...
                        url = "PatientDocumentImageZoomView.aspx?id=" + id +"&patid=<%= PatientID %>";
                    }
                    openWinMaximize(url);
                }
            }
            ...
            function SaveToGuarantorDoc(id) {
                $.ajax({
                    type: 'POST',
                    url: "PatientDocumentDownload.aspx/SaveToGuarantorDoc",
                    contentType: "application/json;charset=utf-8",
                    datatype: "json",
                    data: "{'mode':'patdoc','id':'" + id +"'}",
    ...
    <telerik:RadToolBar ID="tbarMain" runat="server" Width="100%"
        ...
        <Items>
            ...
            <telerik:RadToolBarButton runat="server">
                <ItemTemplate>
                    &nbsp;&nbsp;&nbsp;
                        <telerik:RadCheckBox runat="server" ID="chkIsCurrentEpisode" AutoPostBack="false" Text="Show just current Episode" OnClientCheckedChanged="ApplyQuickSearch">
                        </telerik:RadCheckBox>
                    &nbsp;&nbsp;&nbsp;
                </ItemTemplate>
            </telerik:RadToolBarButton>
    ...
    <telerik:RadGrid runat="server" ID="grdDocument" OnNeedDataSource="grdDocument_NeedDataSource"
        ...
        <MasterTableView ShowHeader="true" AutoGenerateColumns="False" AllowPaging="false"
            ...
            <Columns>
                ...
                <telerik:GridTemplateColumn UniqueName="SGD" HeaderText="Save">
                    <ItemTemplate>
                        <%# DataBinder.Eval(Container.DataItem, "FileAttachName").ToString() == string.Empty ? string.Empty : string.Format("<a href=\"#\" onclick=\"SaveToGuarantorDoc({0}); return false;\"><img src=\"../../../../../Images/Toolbar/save16.png\" border=\"0\" /></a>",
                                                                            DataBinder.Eval(Container.DataItem, "PatientDocumentID"))%>
                    </ItemTemplate>
                    <HeaderStyle HorizontalAlign="Center" Width="35px" />
                    <ItemStyle HorizontalAlign="Center" />
