...
{
    public partial class ServiceUnitTransDetail : BaseUserControl
    {
        ...
        protected override void OnDataBinding(EventArgs e)
        {
            ...
            {
                ViewState["ToServiceUnitID" + Request.UserHostName] = DataBinder.Eval(DataItem, TransChargesItemMetadata.ColumnNames.ToServiceUnitID);
            }
            ...
            if (gbridging.Count > 0)
                ViewState["IsBpjs" + Request.UserHostName] = "true";
            else
                ViewState["IsBpjs" + Request.UserHostName] = "false";
            ...
            //ViewState["LocationID" + Request.UserHostName] = unit.LocationID;
            //ViewState["LocationID" + Request.UserHostName] = ((RadComboBox)Helper.FindControlRecursive(Page, "cboLocationID")).SelectedValue;
            ...
            cboItemID.DataSource = PopulateItem((String)DataBinder.Eval(DataItem, TransChargesItemMetadata.ColumnNames.ItemID), true);
            cboItemID.DataBind();
        ...
        protected void customValidator_ServerValidate(object source, ServerValidateEventArgs args)
        {
            ...
            {
                args.IsValid = false;
                ((CustomValidator)source).ErrorMessage = "Invalid item";
                return;
            }
                ...
                if (ViewState["IsNewRecord"].Equals(true))
                {
                    ...
                    if (isExist)
                    {
                        switch (AppSession.Parameter.HealthcareInitialAppsVersion)
                        {
                            case "RSSA":
                            case "RSCH":
                                if (item.SRItemType == "11" || item.SRItemType == "21")
                                {
                                    args.IsValid = false;
        ...
        private void grdTariff_ItemPreRender(object sender, EventArgs e)
        {
            ...
            // Tariff for Physician
            var cboPhysicianID = (dataItem["TariffComponentName"].FindControl("cboPhysicianID") as RadComboBox);
            cboPhysicianID.Visible = (dataItem["IsTariffParamedic"].Text == "True");

            if (cboPhysicianID.Visible)
            {
                if (AppSession.Parameter.IsDefaultEmptyPhysicianOnTransactionEntry && AppSession.UserLogin.SRUserType!=UserLogin.UserType.Doctor)
                {
                    ...
                    //{
                    //    cboPhysicianID.Items.Add(new RadComboBoxItem((string)row["ParamedicName"], (string)row["ParamedicID"]));
                    //}

                    //cboPhysicianID.SelectedValue = comp != null ? comp.ParamedicID : string.Empty;

                    // cboPhysicianID lookup diganti webservice jadi cukup isi 1 saja dg isi comp.ParamedicID (Handono 2019-11-28)
                    if (comp != null)
                    {
                        ComboBox.PopulateWithOneRow(cboPhysicianID, comp.ParamedicID, Enums.EntityClassName.Paramedic);
                    }
                }
                else
                {
                    ...
                    //cboPhysicianID.DataSource = view;
                    ...
                    //cboPhysicianID.SelectedValue = comp != null ? comp.ParamedicID :
                    //    (isItemLab ? AppSession.Parameter.ParamedicIdLabDefault : Request.QueryString["parid"]);
                    ...
                    if (comp == null)
                    {
                        ...
                        pid = (isItemLab ? AppSession.Parameter.ParamedicIdLabDefault : AppSession.UserLogin.ParamedicID);
                    }
                    else 
                        pid = comp.ParamedicID;

                    // cboPhysicianID lookup diganti webservice jadi cukup isi 1 saja  (Handono 2019-11-28)
                    ComboBox.PopulateWithOneRow(cboPhysicianID, pid, Enums.EntityClassName.Paramedic);
                }
            }
                ...
                if (Request.QueryString["type"] == "tr" || Request.QueryString["type"] == "mcu")
                {
                    ...
                    else if (!string.IsNullOrEmpty(TxtPackageReferenceNo.Text))
                    {
                        ...
                        else
                        {
                            ...
                            query.Where
                            (
                                query.ServiceUnitID == ViewState["ToServiceUnitID" + Request.UserHostName].ToString(),
                                medic.IsActive == true
                            );
                        }
                    }
        ...
        private DataTable PopulateServiceItem(string searchText, bool VisibleOnly)
        {
            ...
            try
            {
                ...
                else
                {
                    query.InnerJoin(itemUnit).On(
                            query.ItemID == itemUnit.ItemID &&
                            itemUnit.ServiceUnitID == ((RadComboBox)Helper.FindControlRecursive(Page, "cboToServiceUnitID")).SelectedValue
                        );
                }
        ...
        private void PopulateTariff(string itemID)
        {
            ...
            {
                coll = Helper.Tariff.GetItemTariffComponentCollection(tariffDate, (string)ViewState["SRTariffType" + Request.UserHostName],
                ...
                tariff = (Helper.Tariff.GetItemTariff(tariffDate, (string)ViewState["SRTariffType" + Request.UserHostName], ((RadTextBox)Helper.FindControlRecursive(Page, "txtClassID")).Text, ((RadTextBox)Helper.FindControlRecursive(Page, "txtClassID")).Text, itemID, (string)ViewState["GuarantorID" + Request.UserHostName], false, reg.SRRegistrationType) ??
                          Helper.Tariff.GetItemTariff(tariffDate, (string)ViewState["SRTariffType" + Request.UserHostName], AppSession.Parameter.DefaultTariffClass, ((RadTextBox)Helper.FindControlRecursive(Page, "txtClassID")).Text, itemID, (string)ViewState["GuarantorID" + Request.UserHostName], false, reg.SRRegistrationType)) ??
                         (Helper.Tariff.GetItemTariff(tariffDate, AppSession.Parameter.DefaultTariffType, ((RadTextBox)Helper.FindControlRecursive(Page, "txtClassID")).Text, ((RadTextBox)Helper.FindControlRecursive(Page, "txtClassID")).Text, itemID, (string)ViewState["GuarantorID" + Request.UserHostName], false, reg.SRRegistrationType) ??
                          Helper.Tariff.GetItemTariff(tariffDate, AppSession.Parameter.DefaultTariffType, AppSession.Parameter.DefaultTariffClass, ((RadTextBox)Helper.FindControlRecursive(Page, "txtClassID")).Text, itemID, (string)ViewState["GuarantorID" + Request.UserHostName], false, reg.SRRegistrationType));
        ...
        private string GetCoveredItem(string itemId)
        {
            ...
            #region it's no longer used
            //if ((string)ViewState["IsBpjs" + Request.UserHostName] == "true")
            //{
            //    bool isGuarantor;
            ...
            //    {
            //        case ItemType.Medical:
            //            isGuarantor = (grr.IsIncludeItemMedical ?? false) && (grr.IsIncludeItemMedicalToGuarantor ?? false);
            //            break;
            //        case ItemType.NonMedical:
            //        case ItemType.Kitchen:
            //            isGuarantor = (grr.IsIncludeItemNonMedical ?? false) && (grr.IsIncludeItemNonMedicalToGuarantor ?? false);
            //            break;
            //        default:
            //            var types = new GuarantorItemTypeRuleCollection();
            //            types.Query.Where(types.Query.GuarantorID == CboGuarantorId.SelectedValue);
            ...
            //    var grrItemRule = new GuarantorItemRule();
            //    if (grrItemRule.LoadByPrimaryKey(CboGuarantorId.SelectedValue, itemId))
            //        isGuarantor = grrItemRule.IsToGuarantor ?? false;

            //    var regItemRule = new BusinessObject.RegistrationItemRule();
            //    if (regItemRule.LoadByPrimaryKey(TxtRegistrationNo.Text, itemId))
            //        isGuarantor = regItemRule.IsToGuarantor ?? false;
