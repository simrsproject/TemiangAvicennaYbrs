    ...
    public partial class TariffComponentPackage : BasePageDialog
    {
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                ViewState["PrimaryKey" + Request.UserHostName] = "|";
        ...
        {
            var list = ((TransChargesItemCollection)Session["collTransChargesItem" + Request.UserHostName]).Where(i => i.TransactionNo == Request.QueryString["trans"] &&
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            {
                case "grdTariff":
                    ViewState["PrimaryKey" + Request.UserHostName] = eventArgument;
        ...
        private void grdTariff_ItemPreRender(object sender, EventArgs e)
        {
            ...
            if (ViewState["SRTariffType" + Request.UserHostName] == null)
            {
            ...
            var str = ViewState["PrimaryKey" + Request.UserHostName].ToString().Split('|');
            var entity = (Session["collTransChargesItem" + Request.UserHostName] as TransChargesItemCollection).FindByPrimaryKey(str[0], str[1]);

            DataTable tbl = Helper.Tariff.GetItemTariffComponent(DateTime.Parse(Request.QueryString["date"]), (string)ViewState["SRTariffType" + Request.UserHostName],
                entity.ChargeClassID, dataItem["TariffComponentID"].Text, entity.ItemID);

            if (tbl.Rows.Count == 0)
                tbl = Helper.Tariff.GetItemTariffComponent(DateTime.Parse(Request.QueryString["date"]), (string)ViewState["SRTariffType" + Request.UserHostName],
            ...
            TransChargesItemComp comp = (((TransChargesItemCompCollection)Session["collTransChargesItemComp" + Request.UserHostName])
        ...
        {
            var str = ViewState["PrimaryKey" + Request.UserHostName].ToString().Split('|');
            var list = from i in ((TransChargesItemCompCollection)Session["collTransChargesItemComp" + Request.UserHostName])
        ...
        protected void grdTariff_ItemCommand(object source, GridCommandEventArgs e)
        {
            if (e.CommandName == "Save")
            {
                ...
                {
                    var detail =
                        ((TransChargesItemCollection)Session["collTransChargesItem" + Request.UserHostName]).FindByPrimaryKey(
                    ...
                    if (detail != null)
                    {
                        ...
                        if (medic.Visible)
                        {
                            if (string.IsNullOrEmpty(detail.ParamedicCollectionName))
                                detail.ParamedicCollectionName = medic.Text;
                            else
                                detail.ParamedicCollectionName += "; " + medic.Text;
                        }
                    }

                    var entity = ((TransChargesItemCompCollection)Session["collTransChargesItemComp" + Request.UserHostName]).FindByPrimaryKey(
        ...
        protected void grdList_DeleteCommand(object source, GridCommandEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(sequenceNo))
            {
                ...
                if (entity != null)
                {
                    foreach (var comp in (Session["collTransChargesItemComp" + Request.UserHostName] as TransChargesItemCompCollection).Where(comp => comp.SequenceNo == entity.SequenceNo))
                    {
                        comp.MarkAsDeleted();
                    }

                    foreach (var cons in (Session["collTransChargesItemConsumption" + Request.UserHostName] as TransChargesItemConsumptionCollection).Where(cons => cons.SequenceNo == entity.SequenceNo))
                    {
                        cons.MarkAsDeleted();
                    }
        ...
        {
            return ((TransChargesItemCollection)Session["collTransChargesItem" + Request.UserHostName]).FirstOrDefault(rec => rec.SequenceNo.Equals(sequenceNo));
        }

        private TransChargesItemComp FindTransChargesItemComp(String sequenceNo, string tariffComponentID)
        {
            return ((TransChargesItemCompCollection)Session["collTransChargesItemComp" + Request.UserHostName]).FirstOrDefault(rec => rec.SequenceNo.Equals(sequenceNo) && rec.TariffComponentID.Equals(tariffComponentID));
        }
    }
}
