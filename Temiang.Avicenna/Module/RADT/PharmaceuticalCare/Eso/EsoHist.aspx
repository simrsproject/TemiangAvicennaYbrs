<%@ Page Language="C#" MasterPageFile="~/MasterPage/MasterDialog.Master" AutoEventWireup="true"
    CodeBehind="EsoHist.aspx.cs" Inherits="Temiang.Avicenna.Module.RADT.PharmaceuticalCare.EsoHist" %>
...
<%--                        var obj = {};
            ...
            function openPrintPreview(method, parameter) {
                $.ajax({
                    url: "<%= Helper.UrlRoot() %>/Module/RADT/Cpoe/EmrWebService.asmx/" + method,
    ...
    <telerik:RadGrid ID="grdList" runat="server" OnNeedDataSource="grdList_NeedDataSource" OnDeleteCommand="grdList_DeleteCommand" OnItemDataBound="grdList_ItemDataBound"
        ...
        AllowSorting="true">
        <MasterTableView DataKeyNames="EsoNo,IsDeleted">
            <Columns>
                <telerik:GridTemplateColumn UniqueName="TemplateItemName3" HeaderStyle-Width="30px">
                    <ItemTemplate>
                        <%# string.Format("<a href=\"#\" onclick=\"javascript:entryDetail('view', '{0}'); return false;\"><img src=\"{1}/Images/Toolbar/views16.png\"  alt=\"New\" /></a>",DataBinder.Eval(Container.DataItem, "EsoNo"),Helper.UrlRoot())%>
                        <br />
                        <br />
                ...
                <telerik:GridBoundColumn DataField="EsoNo" HeaderText="No" UniqueName="EsoNo" SortExpression="EsoNo" Visible="false">
                    <HeaderStyle HorizontalAlign="Center" Width="50px" />
                    <ItemStyle HorizontalAlign="Center" />
                </telerik:GridBoundColumn>
                <telerik:GridTemplateColumn UniqueName="DrugObsDateTime" HeaderText="Time">
                    <ItemTemplate>
                        <%# Convert.ToDateTime(DataBinder.Eval(Container.DataItem, "EsoDateTime")).ToString(AppConstant.DisplayFormat.DateShortMonth) %><br />
                        <%# Convert.ToDateTime(DataBinder.Eval(Container.DataItem, "EsoDateTime")).ToString(AppConstant.DisplayFormat.HourMin) %>
                    </ItemTemplate>
                ...
                <telerik:GridTemplateColumn UniqueName="DrugInteractionRisk" HeaderText="" ItemStyle-VerticalAlign="Top">
                    <ItemTemplate>
                        <fieldset style="width: 400px;">
                            <legend>Main Disease</legend>
                            <%# DataBinder.Eval(Container.DataItem, "MainDisease") %>
                        </fieldset>
                        <fieldset style="width: 400px;">
                            <legend>Drug Side Effects</legend>
                            <%# DataBinder.Eval(Container.DataItem, "EsoManifestations") %>
                        </fieldset>
                        <br />
                        <fieldset style="width: 400px;">
                            <legend>Past History</legend>
                            <%# DataBinder.Eval(Container.DataItem, "PrevEsoHistory") %>
                        </fieldset>
                        <br />
                        <fieldset style="width: 400px;">
