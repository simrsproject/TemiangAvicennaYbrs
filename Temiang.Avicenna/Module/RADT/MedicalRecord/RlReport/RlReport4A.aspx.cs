...
{
    public partial class RlReport4A : BasePageDetail
    {
        ...
        private void SaveEntity(RlTxReport entity)
        {
            ...
            var coll = new RlTxReport4ACollection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = RlTxReport4As;

            RlTxReport4A.Process(fromMonth, toMonth, year, AppSession.Parameter.DischargeConditionDieLessThen48, AppSession.Parameter.DischargeConditionDieMoreThen48, AppSession.UserLogin.UserID, newcoll, out newcoll);
        ...
        protected void grdRlReport4A_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            //grdRlReport4A.DataSource = RlTxReport4As;
        }

        private RlTxReport4ACollection RlTxReport4As
        {
            get
            {
                var obj = ViewState["collRlTxReport4A"];
                if (obj != null)
                    return ((RlTxReport4ACollection)(obj));

                var collection = new RlTxReport4ACollection();
                ...
                ViewState["collRlTxReport4A"] = collection;

                return collection;
            }
            set { ViewState["collRlTxReport4A"] = value; }
        }
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            switch (eventArgument)
            {
                case "process":
                    ...
                    grdRlReport4A.Rebind();

                    break;
                case "print":
                    Print(AppConstant.Report.RL4A);
                    break;
            }
        }
        ...
        private void Process()
        {
            ...
            epidiagq.InnerJoin(diagq).On(
                epidiagq.DiagnoseID == diagq.DiagnoseID &&
                diagq.DtdNo.In(RlTxReport4As.Select(r => r.RlMasterReportItemCode).ToArray())
                );
            epidiagq.InnerJoin(regq).On(epidiagq.RegistrationNo == regq.RegistrationNo);
            epidiagq.InnerJoin(patq).On(regq.PatientID == patq.PatientID);
            epidiagq.Where(
            ...
            foreach (DataRow row in dtb.Rows)
            {
                var item = RlTxReport4As.Where(r => r.RlMasterReportItemCode == row["DtdNo"].ToString()).Single();
        ...
        private void RefreshCommandItemGrid(AppEnum.DataMode oldVal, AppEnum.DataMode newVal)
        {
            ...
            grdRlReport4A.MasterTableView.CommandItemDisplay = isVisible ? GridCommandItemDisplay.Top : GridCommandItemDisplay.None;
