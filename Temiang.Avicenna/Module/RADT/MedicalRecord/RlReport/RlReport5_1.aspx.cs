...
namespace Temiang.Avicenna.Module.RADT.MedicalRecord
{
    public partial class RlReport5_1 : BasePageDetail
    {
        ...
        {
            var coll = new RlTxReport51Collection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = new RlTxReport51Collection();

            foreach (GridDataItem dataItem in grdRlReport5_1.MasterTableView.Items)
            {
        ...
        {
            grdRlReport5_1.DataSource = RlTxReport51S;
        }

        private RlTxReport51Collection RlTxReport51S
        {
            get
            {
                var obj = ViewState["collRlTxReport51"];
                if (obj != null)
                    return ((RlTxReport51Collection)(obj));

                var collection = new RlTxReport51Collection();

                var query = new RlTxReport51Query("a");
                ...
                ViewState["collRlTxReport51"] = collection;

                return collection;
            }
            set { ViewState["collRlTxReport51"] = value; }
        }
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            switch (eventArgument)
            {
                case "process":
                    ...
                    break;
                case "print":
                    Print(AppConstant.Report.RL5_1);
                    break;
            }
        }
        ...
        private void Process()
        {
            ...
            foreach (var item in RlTxReport51S)
            {
                var mri = new RlMasterReportItem();
                mri.LoadByPrimaryKey(item.RlMasterReportItemID ?? 0);

                RlTxReport51.Process(fromMonth, toMonth, year, mri.RlMasterReportItemCode, out int jml);
