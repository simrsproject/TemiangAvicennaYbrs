...
namespace Temiang.Avicenna.Module.RADT.MedicalRecord
{
    public partial class RlReport3_5 : BasePageDetail
    {
        ...
        {
            var coll = new RlTxReport35Collection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = new RlTxReport35Collection();
        ...
        {
            grdRlReport3_5.DataSource = RlTxReport35S;
        }

        private RlTxReport35Collection RlTxReport35S
        {
            get
            {
                var obj = ViewState["collRlTxReport35"];
                if (obj != null)
                    return ((RlTxReport35Collection)(obj));

                var collection = new RlTxReport35Collection();

                var query = new RlTxReport35Query("a");
                ...
                ViewState["collRlTxReport35"] = collection;

                return collection;
            }
            set { ViewState["collRlTxReport35"] = value; }
        }
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            switch (eventArgument)
            {
                case "process":
                    ...
                    break;
                case "print":
                    Print(AppConstant.Report.RL3_5);
                    break;
            }
        }
        ...
        private void Process()
        {
            ...
            foreach (var item in RlTxReport35S)
            {
                RlTxReport35.Process(fromMonth, toMonth, year, item.RlMasterReportItemID ?? 0, out int pRmRumahSakit, out int pRmBidan, out int pRmPuskesmas, out int pRmFasKesLain,
