...
{
    public partial class RlReport5_2Detail : BaseUserControl
    {
        ...
        protected override void OnDataBinding(EventArgs e)
        {
            ...
            qDiag.Where(qDiag.DiagnoseID == (String)DataBinder.Eval(DataItem, RlTxReport52V2025Metadata.ColumnNames.DiagnosaID));
            DataTable dtb = qDiag.LoadDataTable();

            cboDiagnoseID.DataSource = dtb;
            cboDiagnoseID.DataBind();

            cboDiagnoseID.SelectedValue = (String)DataBinder.Eval(DataItem, RlTxReport52V2025Metadata.ColumnNames.DiagnosaID);
            txtKasusBaruL.Value = Convert.ToDouble(DataBinder.Eval(DataItem, RlTxReport52V2025Metadata.ColumnNames.KasusBaruL));
            txtKasusBaruP.Value = Convert.ToDouble(DataBinder.Eval(DataItem, RlTxReport52V2025Metadata.ColumnNames.KasusBaruP));
            txtJumlahKasusBaru.Value = Convert.ToDouble(DataBinder.Eval(DataItem, RlTxReport52V2025Metadata.ColumnNames.JumlahKasusBaru));
            txtKunjunganL.Value = Convert.ToDouble(DataBinder.Eval(DataItem, RlTxReport52V2025Metadata.ColumnNames.KunjunganL));
            txtKunjunganP.Value = Convert.ToDouble(DataBinder.Eval(DataItem, RlTxReport52V2025Metadata.ColumnNames.KunjunganP));
            txtJumlahKunjungan.Value = Convert.ToDouble(DataBinder.Eval(DataItem, RlTxReport52V2025Metadata.ColumnNames.JumlahKunjungan));
        }
        ...
        protected void customValidator_ServerValidate(object source, ServerValidateEventArgs args)
        {
            ...
            {
                RlTxReport52V2025Collection coll =
                    (RlTxReport52V2025Collection)Session["collRlTxReport52"];

                string sValue = cboDiagnoseID.SelectedValue;
                bool isExist = false;
                foreach (RlTxReport52V2025 item in coll)
                {
