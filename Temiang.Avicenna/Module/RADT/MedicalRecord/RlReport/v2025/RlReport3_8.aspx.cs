...
{
    public partial class RlReport3_8 : BasePageDetail
    {
        ...
        {
            var coll = new RlTxReport38V2025Collection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = new RlTxReport38V2025Collection();

            foreach (GridDataItem dataItem in grdRlReport3_8.MasterTableView.Items)
            {
                var reportItemId = dataItem.GetDataKeyValue("RlMasterReportItemID").ToInt();
                var txtJumlahPemeriksaanL = (dataItem.FindControl("txtJumlahPemeriksaanL") as RadNumericTextBox);
                var txtJumlahPemeriksaanP = (dataItem.FindControl("txtJumlahPemeriksaanP") as RadNumericTextBox);

                var rpt = newcoll.AddNew();
                rpt.RlTxReportNo = entity.RlTxReportNo;
                rpt.RlMasterReportItemID = reportItemId;
                rpt.JumlahPemeriksaanL = txtJumlahPemeriksaanL.Value.ToInt();
                rpt.JumlahPemeriksaanP = txtJumlahPemeriksaanP.Value.ToInt();
        ...
        protected void grdRlReport3_8_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            grdRlReport3_8.DataSource = RlTxReport38S;
        }

        private RlTxReport38V2025Collection RlTxReport38S
        {
            get
            {
                var obj = ViewState["collRlTxReport38V2025"];
                if (obj != null)
                    return ((RlTxReport38V2025Collection)(obj));

                var collection = new RlTxReport38V2025Collection();

                var query = new RlTxReport38V2025Query("a");
                ...
                query.OrderBy(mriq.RlMasterReportItemID.Ascending);
                ...
                if (collection.Count == 0)
                {
                    ...
                    foreach (var item in mric)
                    {
                        ...
                        coll.JumlahPemeriksaanL = 0;
                        coll.JumlahPemeriksaanP = 0;
                        coll.LastUpdateByUserID = AppSession.UserLogin.UserID;
                        coll.LastUpdateDateTime = DateTime.Now;
                    }
                }

                ViewState["collRlTxReport38V2025"] = collection;

                return collection;
            }
            set { ViewState["collRlTxReport38V2025"] = value; }
        }
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            switch (eventArgument)
            {
                case "process":
                    ...
                    grdRlReport3_8.Rebind();

                    break;
                case "print":
                    Print(AppConstant.Report.RL3_8V2025);
                    break;
            }
        }
        ...
        private void Process()
        {
            ...
            foreach (var item in RlTxReport38S)
            {
                var mri = new RlMasterReportItemV2025();
                mri.LoadByPrimaryKey(item.RlMasterReportItemID ?? 0);

                RlTxReport38V2025.Process(fromMonth, toMonth, year, mri.RlMasterReportItemID ?? 0, out int jmlPemeriksaanL, out int jmlPemeriksaanP);

                item.JumlahPemeriksaanL = jmlPemeriksaanL;
                item.JumlahPemeriksaanP = jmlPemeriksaanP;
        ...
        private void RefreshCommandItemGrid(AppEnum.DataMode oldVal, AppEnum.DataMode newVal)
        {
            ...
            grdRlReport3_8.MasterTableView.CommandItemDisplay = isVisible ? GridCommandItemDisplay.Top : GridCommandItemDisplay.None;
