...
namespace Temiang.Avicenna.Module.RADT.MedicalRecord.v2025
{
    public partial class RlReport3_10: BasePageDetail
    {
        ...
        {
            var coll = new RlTxReport310V2025Collection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = new RlTxReport310V2025Collection();
            ...
            foreach (GridDataItem dataItem in grdRlReport3_10.MasterTableView.Items)
            {
                ...
                var txtRujukanPuskesmas = (dataItem.FindControl("txtRujukanPuskesmas") as RadNumericTextBox);
                var txtRujukanRsLain = (dataItem.FindControl("txtRujukanRsLain") as RadNumericTextBox);
                var txtRujukanFasKesLain = (dataItem.FindControl("txtRujukanFasKesLain") as RadNumericTextBox);
                //var txtTotalRujukanMasuk = (dataItem.FindControl("txtTotalRujukanMasuk") as RadNumericTextBox);
                var txtDirujukKePuskesmasAsal = (dataItem.FindControl("txtDirujukKePuskesmasAsal") as RadNumericTextBox);
                var txtDirujukKeRsAsal = (dataItem.FindControl("txtDirujukKeRsAsal") as RadNumericTextBox);
                var txtDirujukKeFasKesAsal = (dataItem.FindControl("txtDirujukKeFasKesAsal") as RadNumericTextBox);
                //var txtTotalRujukanDikembalikan = (dataItem.FindControl("txtTotalRujukanDikembalikan") as RadNumericTextBox);
                var txtDirujukPasienRujukan = (dataItem.FindControl("txtDirujukPasienRujukan") as RadNumericTextBox);
                var txtDirujukPasienDtgSendiri = (dataItem.FindControl("txtDirujukPasienDtgSendiri") as RadNumericTextBox);
                //var txtTotalDirujukKeluar = (dataItem.FindControl("txtTotalDirujukKeluar") as RadNumericTextBox);
                var txtDirujukDiterimaKembali = (dataItem.FindControl("txtDirujukDiterimaKembali") as RadNumericTextBox);

                var rpt = newcoll.AddNew();
                rpt.RlTxReportNo = entity.RlTxReportNo;
                rpt.RlMasterReportItemID = reportItemId;
                rpt.RujukanPuskesmas = txtRujukanPuskesmas.Value.ToInt();
                rpt.RujukanRsLain = txtRujukanRsLain.Value.ToInt();
                rpt.RujukanFasKesLain = txtRujukanFasKesLain.Value.ToInt();
                //rpt.TotalRujukanMasuk = txtTotalRujukanMasuk.Value.ToInt();
                rpt.DirujukKePuskesmasAsal = txtDirujukKePuskesmasAsal.Value.ToInt();
                rpt.DirujukKeRsAsal = txtDirujukKeRsAsal.Value.ToInt();
                rpt.DirujukKeFasKesAsal = txtDirujukKeFasKesAsal.Value.ToInt();
                //rpt.TotalRujukanDikembalikan = txtTotalRujukanDikembalikan.Value.ToInt();
                rpt.DirujukPasienRujukan = txtDirujukPasienRujukan.Value.ToInt();
                rpt.DirujukPasienDtgSendiri = txtDirujukPasienDtgSendiri.Value.ToInt();
                //rpt.TotalDirujukKeluar = txtTotalDirujukKeluar.Value.ToInt();
                rpt.DirujukDiterimaKembali = txtDirujukDiterimaKembali.Value.ToInt();
        ...
        {
            grdRlReport3_10.DataSource = RlTxReport310V2025S;
        }

        private RlTxReport310V2025Collection RlTxReport310V2025S
        {
            get
            {
                var obj = ViewState["collRlTxReport310V2025"];
                if (obj != null)
                    return ((RlTxReport310V2025Collection)(obj));

                var collection = new RlTxReport310V2025Collection();

                var query = new RlTxReport310V2025Query("a");
                ...
                if (collection.Count == 0)
                {
                    ...
                    foreach (var item in mric)
                    {
                        ...
                        coll.RujukanPuskesmas = 0;
                        coll.RujukanRsLain = 0;
                        coll.RujukanFasKesLain = 0;
                        //coll.TotalRujukanMasuk = 0;
                        coll.DirujukKePuskesmasAsal = 0;
                        coll.DirujukKeRsAsal = 0;
                        coll.DirujukKeFasKesAsal = 0;
                        //coll.TotalRujukanDikembalikan = 0;
                        coll.DirujukPasienRujukan = 0;
                        coll.DirujukPasienDtgSendiri = 0;
                        //coll.TotalDirujukKeluar = 0;
                        coll.DirujukDiterimaKembali = 0;
                        coll.LastUpdateByUserID = AppSession.UserLogin.UserID;
                        coll.LastUpdateDateTime = DateTime.Now;
                    }
                }

                ViewState["collRlTxReport310V2025"] = collection;

                return collection;
            }
            set { ViewState["collRlTxReport31V2025"] = value; }
        }
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            switch (eventArgument)
            {
                case "process":
                    ...
                    break;
                case "print":
                    Print(AppConstant.Report.RL3_10V2025);
                    break;
            }
        }
        ...
        private void Process()
        {
            ...
            foreach (var item in RlTxReport310V2025S)
            {
                int rujukanPuskesmas = 0, rujukanRsLain = 0, rujukanFasKesLain = 0, dirujukKePuskesmasAsal = 0, dirujukKeRsAsal = 0, dirujukKeFasKesAsal = 0;
                int dirujukPasienRujukan = 0, dirujukPasienDtgSendiri = 0, dirujukDiterimaKembali = 0;
                ...
                {
                    RlTxReport310V2025.Process(fromMonth, toMonth, year, paramedicRl1, out rujukanPuskesmas, out rujukanRsLain, out rujukanFasKesLain, out dirujukKePuskesmasAsal,
                        out dirujukKeRsAsal, out dirujukKeFasKesAsal, out dirujukPasienRujukan, out dirujukPasienDtgSendiri, out dirujukDiterimaKembali);
                }

                item.RujukanPuskesmas = rujukanPuskesmas;
                item.RujukanRsLain = rujukanRsLain;
                item.RujukanFasKesLain = rujukanFasKesLain;
                //item.TotalRujukanMasuk = totalRujukanMasuk;
