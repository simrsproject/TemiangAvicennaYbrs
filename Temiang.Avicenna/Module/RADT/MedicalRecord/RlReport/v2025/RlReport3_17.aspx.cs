...
{
    public partial class RlReport3_17 : BasePageDetail
    {
        ...
        {
            var coll = new RlTxReport317Collection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = new RlTxReport317Collection();

            foreach (GridDataItem dataItem in grdRlReport3_17.MasterTableView.Items)
            {
        ...
        protected void grdRlReport3_17_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            grdRlReport3_17.DataSource = RlTxReport317S;
        }

        private RlTxReport317Collection RlTxReport317S
        {
            get
            {
                var obj = ViewState["collRlTxReport317"];
                if (obj != null)
                    return ((RlTxReport317Collection)(obj));

                var collection = new RlTxReport317Collection();

                var query = new RlTxReport317Query("a");
                ...
                ViewState["collRlTxReport317"] = collection;

                return collection;
            }
            set { ViewState["collRlTxReport317"] = value; }
        }
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            switch (eventArgument)
            {
                case "process":
                    ...
                    grdRlReport3_17.Rebind();

                    break;
                case "print":
                    Print(AppConstant.Report.RL3_17); //nanti kalo udah selesai ini harus di uncommand
        ...
        private void Process()
        {
            ...
            foreach (var item in RlTxReport317S)
            {
                var mri = new RlMasterReportItemV2025();
                mri.LoadByPrimaryKey(item.RlMasterReportItemID ?? 0);

                RlTxReport317.Process(fromMonth, toMonth, year, mri.RlMasterReportItemCode, out int jmlItemObat, out int jmlItemObatRs);
        ...
        private void RefreshCommandItemGrid(AppEnum.DataMode oldVal, AppEnum.DataMode newVal)
        {
            ...
            grdRlReport3_17.MasterTableView.CommandItemDisplay = isVisible ? GridCommandItemDisplay.Top : GridCommandItemDisplay.None;
