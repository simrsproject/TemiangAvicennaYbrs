...
{
    public partial class RlReport4_3Detail : BaseUserControl
    {
        ...
        protected override void OnDataBinding(EventArgs e)
        {
            ...
            qDiag.Where(qDiag.DiagnoseID == (String)DataBinder.Eval(DataItem, RlTxReport43V2025Metadata.ColumnNames.DiagnosaID));
            DataTable dtb = qDiag.LoadDataTable();

            cboDiagnoseID.DataSource = dtb;
            cboDiagnoseID.DataBind();

            cboDiagnoseID.SelectedValue = (String)DataBinder.Eval(DataItem, RlTxReport43V2025Metadata.ColumnNames.DiagnosaID);
            txtHidupMatiL.Value = Convert.ToDouble(DataBinder.Eval(DataItem, RlTxReport43V2025Metadata.ColumnNames.HidupMatiL));
            txtHidupMatiP.Value = Convert.ToDouble(DataBinder.Eval(DataItem, RlTxReport43V2025Metadata.ColumnNames.HidupMatiP));
            txtKeluarMatiL.Value = Convert.ToDouble(DataBinder.Eval(DataItem, RlTxReport43V2025Metadata.ColumnNames.KeluarMatiL));
            txtKeluarMatiP.Value = Convert.ToDouble(DataBinder.Eval(DataItem, RlTxReport43V2025Metadata.ColumnNames.KeluarMatiP));
        }
        ...
        protected void customValidator_ServerValidate(object source, ServerValidateEventArgs args)
        {
            //Check duplicate key
            {
                RlTxReport43V2025Collection coll =
                    (RlTxReport43V2025Collection)Session["collRlTxReport43V2025"];

                string sValue = cboDiagnoseID.SelectedValue;
                bool isExist = false;
                foreach (RlTxReport43V2025 item in coll)
                {
        ...
        public Decimal HidupMatiL
        {
            get { return Convert.ToDecimal(txtHidupMatiL.Value); }
        }

        public Decimal HidupMatiP
        {
            get { return Convert.ToDecimal(txtHidupMatiP.Value); }
        }
