...
{
    public partial class RlReport3_16 : BasePageDetail
    {
        ...
        {
            var coll = new RlTxReport316V2025Collection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = new RlTxReport316V2025Collection();

            foreach (GridDataItem dataItem in grdRlReport3_16.MasterTableView.Items)
            {
                var reportItemId = dataItem.GetDataKeyValue("RlMasterReportItemID").ToInt();
                var txtPKBPascaPersalinan = (dataItem.FindControl("txtPKBPascaPersalinan") as RadNumericTextBox);
                var txtPKBPascaKeguguran = (dataItem.FindControl("txtPKBPascaKeguguran") as RadNumericTextBox);
                var txtPKBInterval = (dataItem.FindControl("txtPKBInterval") as RadNumericTextBox);
                var txtPKBTotal = (dataItem.FindControl("txtPKBTotal") as RadNumericTextBox);
                var txtKomplikasiKB = (dataItem.FindControl("txtKomplikasiKB") as RadNumericTextBox);
                var txtKegagalanKB = (dataItem.FindControl("txtKegagalanKB") as RadNumericTextBox);
                var txtEfekSamping = (dataItem.FindControl("txtEfekSamping") as RadNumericTextBox);
                var txtDropOut = (dataItem.FindControl("txtDropOut") as RadNumericTextBox);

                var rpt = newcoll.AddNew();
                rpt.RlTxReportNo = entity.RlTxReportNo;
                rpt.RlMasterReportItemID = reportItemId;
                rpt.PKBPascaPersalinan = txtPKBPascaPersalinan.Value.ToInt();
                rpt.PKBPascaKeguguran = txtPKBPascaKeguguran.Value.ToInt();
                rpt.PKBInterval = txtPKBInterval.Value.ToInt();
                rpt.PKBTotal = txtPKBTotal.Value.ToInt();
                rpt.KomplikasiKB = txtKomplikasiKB.Value.ToInt();
                rpt.KegagalanKB = txtKegagalanKB.Value.ToInt();
                rpt.EfekSamping = txtEfekSamping.Value.ToInt();
                rpt.DropOut = txtDropOut.Value.ToInt();
        ...
        protected void grdRlReport3_16_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            grdRlReport3_16.DataSource = RlTxReport316V2025S;
        }

        private RlTxReport316V2025Collection RlTxReport316V2025S
        {
            get
            {
                var obj = ViewState["collRlTxReport316V2025"];
                if (obj != null)
                    return ((RlTxReport316V2025Collection)(obj));

                var collection = new RlTxReport316V2025Collection();

                var query = new RlTxReport316V2025Query("a");
                ...
                if (collection.Count == 0)
                {
                    ...
                    foreach (var item in mric)
                    {
                        ...
                        coll.PKBPascaPersalinan = 0;
                        coll.PKBPascaKeguguran = 0;
                        coll.PKBInterval = 0;
                        coll.PKBTotal = 0;
                        coll.KomplikasiKB = 0;
                        coll.KegagalanKB = 0;
                ...
                ViewState["collRlTxReport316V2025"] = collection;

                return collection;
            }
            set { ViewState["collRlTxReport316V2025"] = value; }
        }
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            switch (eventArgument)
            {
                case "process":
                    ...
                    grdRlReport3_16.Rebind();

                    break;
                case "print":
                    Print(AppConstant.Report.RL3_16V2025);
                    break;
            }
        }
        ...
        private void Process()
        {
            ...
            foreach (var item in RlTxReport316V2025S)
            {
                RlTxReport316V2025.Process(startDate, endDate, item.RlMasterReportItemID ?? 0, out int PKBPascaPersalinan, out int PKBPascaKeguguran, out int PKBInterval,
                    out int PKBTotal, out int KomplikasiKB, out int KegagalanKB, out int EfekSamping, out int DropOut);

                item.PKBPascaPersalinan = PKBPascaPersalinan;
                item.PKBPascaKeguguran = PKBPascaKeguguran;
                item.PKBInterval = PKBInterval;
                item.PKBTotal = PKBTotal;
                item.KomplikasiKB = KomplikasiKB;
                item.KegagalanKB = KegagalanKB;
                item.EfekSamping = EfekSamping;
        ...
        private void RefreshCommandItemGrid(AppEnum.DataMode oldVal, AppEnum.DataMode newVal)
        {
            ...
            grdRlReport3_16.MasterTableView.CommandItemDisplay = isVisible ? GridCommandItemDisplay.Top : GridCommandItemDisplay.None;

            //Reset Detail
            if (oldVal != AppEnum.DataMode.Read)
                RlTxReport316V2025S = null;
