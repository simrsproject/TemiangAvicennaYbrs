...
{
    public partial class RlReport3_6 : BasePageDetail
    {
        ...
        {
            var coll = new RlTxReport36Collection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = new RlTxReport36Collection();

            foreach (GridDataItem dataItem in grdRlReport3_6.MasterTableView.Items)
            {
                var reportItemId = dataItem.GetDataKeyValue("RlMasterReportItemID").ToInt();
                var txtTotal = (dataItem.FindControl("txtTotal") as RadNumericTextBox);
                var txtKhusus = (dataItem.FindControl("txtKhusus") as RadNumericTextBox);
                var txtBesar = (dataItem.FindControl("txtBesar") as RadNumericTextBox);
                var txtSedang = (dataItem.FindControl("txtSedang") as RadNumericTextBox);
                var txtKecil = (dataItem.FindControl("txtKecil") as RadNumericTextBox);

                var rpt = newcoll.AddNew();
                rpt.RlTxReportNo = entity.RlTxReportNo;
                rpt.RlMasterReportItemID = reportItemId;
                rpt.Total = txtTotal.Value.ToInt();
                rpt.Khusus = txtKhusus.Value.ToInt();
                rpt.Besar = txtBesar.Value.ToInt();
                rpt.Sedang = txtSedang.Value.ToInt();
                rpt.Kecil = txtKecil.Value.ToInt();
        ...
        protected void grdRlReport3_6_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            grdRlReport3_6.DataSource = RlTxReport36S;
        }

        private RlTxReport36Collection RlTxReport36S
        {
            get
            {
                var obj = ViewState["collRlTxReport36"];
                if (obj != null)
                    return ((RlTxReport36Collection)(obj));

                var collection = new RlTxReport36Collection();

                var query = new RlTxReport36Query("a");
                ...
                ViewState["collRlTxReport36"] = collection;

                return collection;
            }
            set { ViewState["collRlTxReport36"] = value; }
        }
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            switch (eventArgument)
            {
                case "process":
                    ...
                    grdRlReport3_6.Rebind();

                    break;
                case "print":
                    Print(AppConstant.Report.RL3_6);
                    break;
            }
        }
        ...
        private void Process()
        {
            ...
            foreach (var item in RlTxReport36S)
            {
                ...
                {
                    RlTxReport36.Process(fromMonth, toMonth, year, surgerySpecialty, out khusus, out besar, out sedang, out kecil, out lain);
                }
        ...
        private void RefreshCommandItemGrid(AppEnum.DataMode oldVal, AppEnum.DataMode newVal)
        {
            ...
            grdRlReport3_6.MasterTableView.CommandItemDisplay = isVisible ? GridCommandItemDisplay.Top : GridCommandItemDisplay.None;
