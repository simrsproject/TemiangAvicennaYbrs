...
namespace Temiang.Avicenna.Module.RADT.MedicalRecord
{
    public partial class RlReport3_3 : BasePageDetail
    {
        ...
        {
            var coll = new RlTxReport33Collection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = new RlTxReport33Collection();
        ...
        {
            grdRlReport3_3.DataSource = RlTxReport33S;
        }

        private RlTxReport33Collection RlTxReport33S
        {
            get
            {
                var obj = ViewState["collRlTxReport33"];
                if (obj != null)
                    return ((RlTxReport33Collection)(obj));

                var collection = new RlTxReport33Collection();

                var query = new RlTxReport33Query("a");
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            switch (eventArgument)
            {
                case "process":
                    ...
                    break;
                case "print":
                    Print(AppConstant.Report.RL3_3);
                    break;
            }
        }
        ...
        private void Process()
        {
            ...
            foreach (var item in RlTxReport33S)
            {
                RlTxReport33.Process(fromMonth, toMonth, year, item.RlMasterReportItemID ?? 0, out int jml);
