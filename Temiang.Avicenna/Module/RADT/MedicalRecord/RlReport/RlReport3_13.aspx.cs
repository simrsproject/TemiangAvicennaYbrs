...
{
    public partial class RlReport3_13 : BasePageDetail
    {
        ...
        {
            var coll = new RlTxReport313Collection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = new RlTxReport313Collection();

            foreach (GridDataItem dataItem in grdRlReport3_13.MasterTableView.Items)
            {
                ...
                var txtRd = (dataItem.FindControl("txtRd") as RadNumericTextBox);
                var txtRi = (dataItem.FindControl("txtRi") as RadNumericTextBox);

                var rpt = newcoll.AddNew();
                rpt.RlTxReportNo = entity.RlTxReportNo;
                rpt.RlMasterReportItemID = reportItemId;
                rpt.Rj = txtRj.Value.ToInt();
                rpt.Rd = txtRd.Value.ToInt();
                rpt.Ri = txtRi.Value.ToInt();
        ...
        protected void grdRlReport3_13_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            grdRlReport3_13.DataSource = RlTxReport313S;
        }

        private RlTxReport313Collection RlTxReport313S
        {
            get
            {
                var obj = ViewState["collRlTxReport313"];
                if (obj != null)
                    return ((RlTxReport313Collection)(obj));

                var collection = new RlTxReport313Collection();

                var query = new RlTxReport313Query("a");
                ...
                ViewState["collRlTxReport313"] = collection;

                return collection;
            }
            set { ViewState["collRlTxReport313"] = value; }
        }
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            switch (eventArgument)
            {
                case "process":
                    ...
                    grdRlReport3_13.Rebind();

                    break;
                case "print":
                    Print(AppConstant.Report.RL3_13);
                    break;
            }
        }
        ...
        private void Process()
        {
            ...
            foreach (var item in RlTxReport313S)
            {
                var mri = new RlMasterReportItem();
                mri.LoadByPrimaryKey(item.RlMasterReportItemID ?? 0);

                RlTxReport313.Process(fromMonth, toMonth, year, mri.RlMasterReportItemCode, out int rj, out int rd, out int ri);
        ...
        private void RefreshCommandItemGrid(AppEnum.DataMode oldVal, AppEnum.DataMode newVal)
        {
            ...
            grdRlReport3_13.MasterTableView.CommandItemDisplay = isVisible ? GridCommandItemDisplay.Top : GridCommandItemDisplay.None;
