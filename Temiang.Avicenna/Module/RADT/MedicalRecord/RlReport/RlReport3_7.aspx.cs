...
namespace Temiang.Avicenna.Module.RADT.MedicalRecord
{
    public partial class RlReport3_7 : BasePageDetail
    {
        ...
        {
            var coll = new RlTxReport37Collection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = new RlTxReport37Collection();

            foreach (GridDataItem dataItem in grdRlReport3_7.MasterTableView.Items)
            {
        ...
        protected void grdRlReport3_7_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            grdRlReport3_7.DataSource = RlTxReport37S;
        }

        private RlTxReport37Collection RlTxReport37S
        {
            get
            {
                var obj = ViewState["collRlTxReport37"];
                if (obj != null)
                    return ((RlTxReport37Collection)(obj));

                var collection = new RlTxReport37Collection();

                var query = new RlTxReport37Query("a");
                ...
                ViewState["collRlTxReport37"] = collection;

                return collection;
            }
            set { ViewState["collRlTxReport37"] = value; }
        }
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            switch (eventArgument)
            {
                case "process":
                    ...
                    grdRlReport3_7.Rebind();

                    break;
                case "print":
                    Print(AppConstant.Report.RL3_7);
                    break;
            }
        }
        ...
        private void Process()
        {
            ...
            foreach (var item in RlTxReport37S)
            {
                RlTxReport37.Process(fromMonth, toMonth, year, item.RlMasterReportItemID ?? 0, out int jml);
        ...
        private void RefreshCommandItemGrid(AppEnum.DataMode oldVal, AppEnum.DataMode newVal)
        {
            ...
            grdRlReport3_7.MasterTableView.CommandItemDisplay = isVisible ? GridCommandItemDisplay.Top : GridCommandItemDisplay.None;
