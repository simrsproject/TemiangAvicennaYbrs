...
{
    public partial class RlReport3_13b : BasePageDetail
    {
        ...
        {
            var coll = new RlTxReport313bCollection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = new RlTxReport313bCollection();

            foreach (GridDataItem dataItem in grdRlReport3_13b.MasterTableView.Items)
            {
                var reportItemId = dataItem.GetDataKeyValue("RlMasterReportItemID").ToInt();
                var txtJumlahItemObat = (dataItem.FindControl("txtJumlahItemObat") as RadNumericTextBox);
                var txtJumlahItemObatRs = (dataItem.FindControl("txtJumlahItemObatRs") as RadNumericTextBox);
                var txtJumlahItemObatFormulariumRs = (dataItem.FindControl("txtJumlahItemObatFormulariumRs") as RadNumericTextBox);

                var rpt = newcoll.AddNew();
                rpt.RlTxReportNo = entity.RlTxReportNo;
                rpt.RlMasterReportItemID = reportItemId;
                rpt.JumlahItemObat = txtJumlahItemObat.Value.ToInt();
                rpt.JumlahItemObatRs = txtJumlahItemObatRs.Value.ToInt();
                rpt.JumlahItemObatFormulariumRs = txtJumlahItemObatFormulariumRs.Value.ToInt();
        ...
        protected void grdRlReport3_13b_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            grdRlReport3_13b.DataSource = RlTxReport313bS;
        }

        private RlTxReport313bCollection RlTxReport313bS
        {
            get
            {
                var obj = ViewState["collRlTxReport313b"];
                if (obj != null)
                    return ((RlTxReport313bCollection)(obj));

                var collection = new RlTxReport313bCollection();

                var query = new RlTxReport313bQuery("a");
                ...
                if (collection.Count == 0)
                {
                    ...
                    foreach (var item in mric)
                    {
                        ...
                        coll.JumlahItemObat = 0;
                        coll.JumlahItemObatRs = 0;
                        coll.JumlahItemObatFormulariumRs = 0;
                        coll.LastUpdateByUserID = AppSession.UserLogin.UserID;
                        coll.LastUpdateDateTime = DateTime.Now;
                    }
                }

                ViewState["collRlTxReport313b"] = collection;

                return collection;
            }
            set { ViewState["collRlTxReport313b"] = value; }
        }
        ...
        protected override void RaisePostBackEvent(IPostBackEventHandler source, string eventArgument)
        {
            ...
            switch (eventArgument)
            {
                case "process":
                    ...
                    grdRlReport3_13b.Rebind();

                    break;
                case "print":
                    Print(AppConstant.Report.RL3_13b);
                    break;
            }
        }
        ...
        private void Process()
        {
            ...
            foreach (var item in RlTxReport313bS)
            {
        ...
        private void RefreshCommandItemGrid(AppEnum.DataMode oldVal, AppEnum.DataMode newVal)
        {
            ...
            grdRlReport3_13b.MasterTableView.CommandItemDisplay = isVisible ? GridCommandItemDisplay.Top : GridCommandItemDisplay.None;

            //Reset Detail
            if (oldVal != AppEnum.DataMode.Read)
                RlTxReport313bS = null;
