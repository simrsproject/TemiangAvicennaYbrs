...
namespace Temiang.Avicenna.Module.RADT.MedicalRecord
{
    public partial class RlReport3_8 : BasePageDetail
    {
        ...
        {
            var coll = new RlTxReport38Collection();
            coll.Query.Where(coll.Query.RlTxReportNo == entity.RlTxReportNo);
            coll.LoadAll();
            coll.MarkAllAsDeleted();
            coll.Save();

            var newcoll = new RlTxReport38Collection();
        ...
        private RlTxReport38Collection RlTxReport38S
        {
            get
            {
                var obj = ViewState["collRlTxReport38"];
                if (obj != null)
                    return ((RlTxReport38Collection)(obj));

                var collection = new RlTxReport38Collection();

                var query = new RlTxReport38Query("a");
                ...
                ViewState["collRlTxReport38"] = collection;

                return collection;
            }
            set { ViewState["collRlTxReport38"] = value; }
        }
        ...
        private void Process()
        {
            ...
            {
                RlTxReport38.Process(fromMonth, toMonth, year, item.RlMasterReportItemID ?? 0, out int jml);
