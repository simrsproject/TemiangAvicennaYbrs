...
namespace Temiang.Avicenna.Module.RADT.Master
{
    public partial class RlMasterReportItemDetail : BaseUserControl
    {
        ...
        protected override void OnDataBinding(EventArgs e)
        {
            ...
            txtRlMasterReportItemID.Value = Convert.ToDouble(DataBinder.Eval(DataItem, RlMasterReportItemMetadata.ColumnNames.RlMasterReportItemID));
            txtRlMasterReportItemNo.Text = (String)DataBinder.Eval(DataItem, RlMasterReportItemMetadata.ColumnNames.RlMasterReportItemNo);
            txtRlMasterReportItemCode.Text = (String)DataBinder.Eval(DataItem, RlMasterReportItemMetadata.ColumnNames.RlMasterReportItemCode);
            txtRlMasterReportItemName.Text = (String)DataBinder.Eval(DataItem, RlMasterReportItemMetadata.ColumnNames.RlMasterReportItemName);
            chkIsActive.Checked = (bool)DataBinder.Eval(DataItem, RlMasterReportItemMetadata.ColumnNames.IsActive);
            cboSRParamedicRL1.SelectedValue = (String)DataBinder.Eval(DataItem, RlMasterReportItemMetadata.ColumnNames.SRParamedicRL1);
            txtParameterValue.Text = (String)DataBinder.Eval(DataItem, RlMasterReportItemMetadata.ColumnNames.ParameterValue);
        }
        protected void customValidator_ServerValidate(object source, ServerValidateEventArgs args)
        {
            ...
            {
                RlMasterReportItemCollection coll =
                    (RlMasterReportItemCollection)Session["collRlMasterReportItem"];

                //TODO: Betulkan cara pengecekannya
                string id = txtRlMasterReportItemID.Text;
                bool isExist = false;
                foreach (RlMasterReportItem item in coll)
                {
