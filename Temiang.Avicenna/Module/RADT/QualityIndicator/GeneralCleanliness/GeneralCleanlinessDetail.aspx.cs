...
{
    public partial class GeneralCleanlinessDetail : BasePageDetail
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            UrlPageList = "GeneralCleanlinessList.aspx";          
        ...
        {
            OnPopulateEntryControl(new GeneralCleanliness());
        ...
        {                       
            var entity = new GeneralCleanliness();
            entity.AddNew();
        ...
        protected override void OnMenuAuditLogClick(AuditLogFilter auditLogFilter)
        {
            ...
            auditLogFilter.TableName = "GeneralCleanliness";
        }
        ...
        private bool IsApprovedOrVoid(GeneralCleanliness entity, ValidateArgs args)
        {
        ...
        {
            RefreshCommandGeneralCleanliness(newVal);            
        }
        ...
        {
            var hh = (GeneralCleanliness)entity;           
            ...
            //PopulateItemGrid();
            PopulateGeneralCleanlinessItemGrid();
        ...
        private void SetApproval(GeneralCleanliness entity, bool isApproval, ValidateArgs args)
        {
        ...
        private void SetVoid(GeneralCleanliness entity, bool isVoid)
        {
            //header
        ...
        private void SetEntityValue(GeneralCleanliness entity)
        {
        ...
        private void SaveEntity(GeneralCleanliness entity)
        {
            var generalCleanlinessItem = new GeneralCleanlinessItemCollection();
            generalCleanlinessItem.Query.Where(generalCleanlinessItem.Query.TransactionNo == entity.TransactionNo);
            generalCleanlinessItem.LoadAll();

            foreach (GridDataItem dataItem in grdItem.MasterTableView.Items)
            {
                string Stdr = dataItem.GetDataKeyValue("SRGeneralCleanliness").ToString();               
                ...
                foreach (GeneralCleanlinessItem row in generalCleanlinessItem)
                {
                    if (row.SRGeneralCleanliness.Equals(Stdr))
                    {                       
                        if (!isYes)
                            row.MarkAsDeleted();
                        if (!isNotApplicable)
                    ...
                    GeneralCleanlinessItem item = generalCleanlinessItem.AddNew();

                    item.TransactionNo = entity.TransactionNo;
                    item.SRGeneralCleanliness = Stdr.ToString();
        ...
        {
            var que = new GeneralCleanlinessQuery("a");
        ...
        #region Record Detail Method Function of GeneralCleanlinessItem       
        private void PopulateGeneralCleanlinessItemGrid()
        {
            grdItem.DataSource = GetGeneralCleanlinessItem();
            grdItem.DataBind();
        }
        ...
        private DataTable GetGeneralCleanlinessItem()
        {
            var query = new GeneralCleanlinessItemQuery("a");
            var qrS = new AppStandardReferenceItemQuery("b");
            if (this.DataModeCurrent == AppEnum.DataMode.Read)
            {
                query.InnerJoin(qrS).On(query.SRGeneralCleanliness == qrS.ItemID);
                query.Where(query.TransactionNo == txtTransactionNo.Text);
            }
            else
            {
                query.RightJoin(qrS).On(query.SRGeneralCleanliness == qrS.ItemID && query.TransactionNo == txtTransactionNo.Text);
            }
            query.Where(qrS.StandardReferenceID == "GeneralCleanliness");
            query.OrderBy(qrS.ItemID.Ascending);
            query.Select
                (
                qrS.ItemID.As("SRGeneralCleanliness"),
                qrS.ItemName.As("GeneralCleanlinessName"),
        ...
        private void RefreshCommandGeneralCleanliness(AppEnum.DataMode newVal)
        {
        ...
        {
            _autoNumber = Helper.GetNewAutoNumber(txtTransactionDate.SelectedDate.Value, AppEnum.AutoNumber.GeneralCleanlinessNo);
