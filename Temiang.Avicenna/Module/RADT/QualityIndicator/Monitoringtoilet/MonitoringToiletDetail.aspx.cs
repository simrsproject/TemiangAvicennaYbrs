...
{
    public partial class MonitoringToiletDetail : BasePageDetail
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            UrlPageList = "MonitoringToiletList.aspx";
        ...
        {
            OnPopulateEntryControl(new MonitoringToilet());
        ...
        {
            var entity = new MonitoringToilet();
            entity.AddNew();
        ...
        protected override void OnMenuAuditLogClick(AuditLogFilter auditLogFilter)
        {
            ...
            auditLogFilter.TableName = "MonitoringToilet";
        }
        ...
        private bool IsApprovedOrVoid(MonitoringToilet entity, ValidateArgs args)
        {
        ...
        {
            RefreshCommandMonitoringToilet(newVal);
        }
        ...
        {
            var hh = (MonitoringToilet)entity;
            ...
            //PopulateItemGrid();
            PopulateMonitoringToiletItemGrid();
        ...
        private void SetApproval(MonitoringToilet entity, bool isApproval, ValidateArgs args)
        {
        ...
        private void SetVoid(MonitoringToilet entity, bool isVoid)
        {
            //header
        ...
        private void SetEntityValue(MonitoringToilet entity)
        {
        ...
        private void SaveEntity(MonitoringToilet entity)
        {
            var monitoringToiletItem = new MonitoringToiletItemCollection();
            monitoringToiletItem.Query.Where(monitoringToiletItem.Query.TransactionNo == entity.TransactionNo);
            monitoringToiletItem.LoadAll();

            foreach (GridDataItem dataItem in grdItem.MasterTableView.Items)
            {
                string Stdr = dataItem.GetDataKeyValue("SRMonitoringToilet").ToString();
                ...
                foreach (MonitoringToiletItem row in monitoringToiletItem)
                {
                    if (row.SRMonitoringToilet.Equals(Stdr))
                    {
                        if (!isYes)
                            row.MarkAsDeleted();
                        if (!isNotApplicable)
                ...
                MonitoringToiletItem item = monitoringToiletItem.AddNew();

                item.TransactionNo = entity.TransactionNo;
                item.SRMonitoringToilet = Stdr.ToString();
        ...
        {
            var que = new MonitoringToiletQuery("a");
        ...
        #region Record Detail Method Function of MonitoringToiletItem       
        private void PopulateMonitoringToiletItemGrid()
        {
            grdItem.DataSource = GetMonitoringToiletItem();
            grdItem.DataBind();
        }
        ...
        private DataTable GetMonitoringToiletItem()
        {
            var query = new MonitoringToiletItemQuery("a");
            var qrS = new AppStandardReferenceItemQuery("b");
            if (this.DataModeCurrent == AppEnum.DataMode.Read)
            {
                query.InnerJoin(qrS).On(query.SRMonitoringToilet == qrS.ItemID);
                query.Where(query.TransactionNo == txtTransactionNo.Text);
            }
            else
            {
                query.RightJoin(qrS).On(query.SRMonitoringToilet == qrS.ItemID && query.TransactionNo == txtTransactionNo.Text);
            }
            query.Where(qrS.StandardReferenceID == "MonitoringToilet");
            query.OrderBy(qrS.ItemID.Ascending);
            query.Select
                (
                qrS.ItemID.As("SRMonitoringToilet"),
                qrS.ItemName.As("MonitoringToiletName"),
        ...
        private void RefreshCommandMonitoringToilet(AppEnum.DataMode newVal)
        {
        ...
        {
            _autoNumber = Helper.GetNewAutoNumber(txtTransactionDate.SelectedDate.Value, AppEnum.AutoNumber.MonitoringToiletNo);
