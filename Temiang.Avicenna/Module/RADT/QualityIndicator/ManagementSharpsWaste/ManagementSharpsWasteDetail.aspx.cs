...
{
    public partial class ManagementSharpsWasteDetail : BasePageDetail
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            UrlPageList = "ManagementSharpsWasteList.aspx";
        ...
        {
            OnPopulateEntryControl(new ManagementSharpsWaste());
        ...
        {
            var entity = new ManagementSharpsWaste();
            entity.AddNew();
        ...
        protected override void OnMenuAuditLogClick(AuditLogFilter auditLogFilter)
        {
            ...
            auditLogFilter.TableName = "ManagementSharpsWaste";
        }
        ...
        private bool IsApprovedOrVoid(ManagementSharpsWaste entity, ValidateArgs args)
        {
        ...
        {
            RefreshCommandManagementSharpsWaste(newVal);
        }
        ...
        {
            var hh = (ManagementSharpsWaste)entity;
            ...
            //PopulateItemGrid();
            PopulateManagementSharpsWasteItemGrid();
        ...
        private void SetApproval(ManagementSharpsWaste entity, bool isApproval, ValidateArgs args)
        {
        ...
        private void SetVoid(ManagementSharpsWaste entity, bool isVoid)
        {
            //header
        ...
        private void SetEntityValue(ManagementSharpsWaste entity)
        {
        ...
        private void SaveEntity(ManagementSharpsWaste entity)
        {
            var managementSharpsWaste = new ManagementSharpsWasteItemCollection();
            managementSharpsWaste.Query.Where(managementSharpsWaste.Query.TransactionNo == entity.TransactionNo);
            managementSharpsWaste.LoadAll();

            foreach (GridDataItem dataItem in grdItem.MasterTableView.Items)
            {
                string Stdr = dataItem.GetDataKeyValue("SRManagementSharpsWaste").ToString();
                ...
                foreach (ManagementSharpsWasteItem row in managementSharpsWaste)
                {
                    if (row.SRManagementSharpsWaste.Equals(Stdr))
                    {
                        if (!isYes)
                            row.MarkAsDeleted();
                        if (!isNotApplicable)
                ...
                ManagementSharpsWasteItem item = managementSharpsWaste.AddNew();

                item.TransactionNo = entity.TransactionNo;
                item.SRManagementSharpsWaste = Stdr.ToString();
        ...
        {
            var que = new ManagementSharpsWasteQuery("a");
        ...
        #region Record Detail Method Function of ManagementSharpsWasteItem       
        private void PopulateManagementSharpsWasteItemGrid()
        {
            grdItem.DataSource = GetManagementSharpsWasteItem();
            grdItem.DataBind();
        }
        ...
        private DataTable GetManagementSharpsWasteItem()
        {
            var query = new ManagementSharpsWasteItemQuery("a");
            var qrS = new AppStandardReferenceItemQuery("b");
            if (this.DataModeCurrent == AppEnum.DataMode.Read)
            {
                query.InnerJoin(qrS).On(query.SRManagementSharpsWaste == qrS.ItemID);
                query.Where(query.TransactionNo == txtTransactionNo.Text);
            }
            else
            {
                query.RightJoin(qrS).On(query.SRManagementSharpsWaste == qrS.ItemID && query.TransactionNo == txtTransactionNo.Text);
            }
            query.Where(qrS.StandardReferenceID == "ManagementSharpsWaste");
            query.OrderBy(qrS.ItemID.Ascending);
            query.Select
                (
                qrS.ItemID.As("SRManagementSharpsWaste"),
                qrS.ItemName.As("ManagementSharpsWasteName"),
        ...
        private void RefreshCommandManagementSharpsWaste(AppEnum.DataMode newVal)
        {
        ...
        {
            _autoNumber = Helper.GetNewAutoNumber(txtTransactionDate.SelectedDate.Value, AppEnum.AutoNumber.ManagementSharpsWasteNo);
