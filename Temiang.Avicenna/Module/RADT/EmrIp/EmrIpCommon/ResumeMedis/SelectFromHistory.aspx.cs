    ...
    public partial class SelectFromHistory : BasePageDialog
    {
        ...
        protected void grdList_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            if (AppParameter.IsYes(AppParameter.ParameterItem.IsMedicalDischargeSummaryPrescJustItemName))
                grdList.DataSource = PrescriptionHistJustItemName(MergeRegistrations);
            else
                grdList.DataSource = PrescriptionHistWithConsumeMethodInfo(MergeRegistrations);
        }
        ...
        private List<SelectedItem> PrescriptionHistJustItemName(List<string> mergeRegistrations)
        {
            ...
            var itemIntervention = new ItemQuery("intv");
            ...
            {
                listItems.Add(new SelectedItem(no, string.Format("{0}", row["ItemName"])));
                no++;
            }
        ...
        #region Prescription History with Consume Method Info
        private List<SelectedItem> PrescriptionHistWithConsumeMethodInfo(List<string> mergeRegistrations)
        {
            var listItems = new List<SelectedItem>();
        ...
        private TransPrescriptionItemQuery PrescriptionItemNameList(List<string> mergeRegistrations, bool isCompound)
        {
            ...
            query.Select(
                "<COALESCE(intv.ItemName, i.ItemName) as ItemName>",
