...
using System.IO;
using static Temiang.Avicenna.Common.BPJS.Applicare.ReferensiKelas;
...
{
    /// <summary>
    /// Discharge Summary Inpatient for DPJP & Casemix 
    /// </summary>
    /// <remarks>
    /// New Fiture :
    ...
    public partial class ResumeMedisRichTextInPatientEntry : BasePageDialogEntry
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            if (IsCallFromCaseMix)
            {
                ...
                IsMedicalRecordEntry = false; //Untuk casemix tidak dibatasi deadline edit & add
            }
            else
            {
...
break;";
        ...
        protected override void OnPopulateEntryControl(ValidateArgs args)
        {
            ...
            else
            {
                if (AppSession.Parameter.HealthcareInitial == "RSI")
                {
                    ...
                    std.LoadByPrimaryKey(AppEnum.StandardReference.Specialty.ToString(), pmedic.SRSpecialty);

                    if (reg.SRRegistrationType == AppConstant.RegistrationType.OutPatient)
                    {
                        var json = new BusinessObject.JsonField.ControlPlan()
                        {
                            Items = new List<BusinessObject.JsonField.ControlPlanItem>()
                        {
                            new BusinessObject.JsonField.ControlPlanItem()
                            {
                                ControlPlanDateTime = Convert.ToDateTime("1/1/0001 12:00:00 AM"),
                                ParamedicID = reg.ParamedicID,
                                ParamedicName = pmedic.ParamedicName,
                                ServiceUnitID = reg.ServiceUnitID,
                                SpecialtyName = std.ItemName
                            }
                        }
                        };
                        controlPlanCtl.Populate(JsonConvert.SerializeObject(json));
                    }
                }
            }
        ...
        public static string LabHist(List<string> mergeRegs)
        {
            ...
            try
            {
                ...
                foreach (DataRow row in dtbLab.Rows)
                {
                    ...
                    // remake karena ada kasus di rsi tidak masuk penutup </ul> nya saat save mds - 20240503 Apip
                    isFirstRow = false;
                    //strb.AppendLine("</ul>");
                }
            }
        ...
        protected override void OnMenuNewClick()
        {
            ...
            if (!medRes.LoadByPrimaryKey(RegistrationNo))
            {
                edtPhysicalExamination.Content = PhysicalExaminationHist(RegistrationNo, FromRegistrationNo);
        ...
        protected override void OnMenuPrintClick(ValidateArgs args, string programID, PrintJobParameterCollection printJobParameters)
        {
            ...
            if (programID.Equals("SLP.01.0089")) //Resume Medis Rawat Inap
            {
        ...
        private void SaveMedicalResume(ValidateArgs args)
        {
            ...
            if (hdnIsNewMds.Value == "1" && !IsCallFromCaseMix)
            {
                        ...
                        if (qSchedule.LoadByPrimaryKey(planItem.ServiceUnitID, planItem.ParamedicID, planItem.ControlPlanDateTime.Year.ToString(), planItem.ControlPlanDateTime.Date))
                        {
                            try
                            {
                                ...
                                var slot = Temiang.Avicenna.WebService.V1_1.AppointmentWS.AppointmentPostRanapSetEntityValue(string.Empty, planItem.ServiceUnitID, planItem.ParamedicID,
                                    ...
                                    pat.MobilePhoneNo, pat.GuarantorCardNo, nosep, 0, AppSession.UserLogin.UserID, AppSession.Parameter.AppointmentTypeControlPlan, null, RegistrationNo);
        ...
        //            //Check appointment sudah ada atau blm supaya tidak terjadi double
        //            //modif by apip 03/01/2024

        //            var apptq = new AppointmentQuery();
        //            apptq.Where(apptq.ServiceUnitID == planItem.ServiceUnitID, apptq.ParamedicID == planItem.ParamedicID,
        //                apptq.PatientID == pat.PatientID, apptq.AppointmentDate == planItem.ControlPlanDateTime.Date);
        //            DataTable apptDtb = apptq.LoadDataTable();
        //            //if (apptDtb.Rows.Count > 0)
        //            //{
        //            //    if (!args.IsCancel && string.IsNullOrWhiteSpace(args.MessageText))
        //            //        args.MessageText = string.Format("Appointment to : {0} for {1} is already exist.", planItem.ParamedicName, planItem.ControlPlanDateTime.ToString("dd-MMM-yyyy"));
        //            //    return;
        //            //}

        //            //db:20240130 - hanya yg belum ada appointment aja yg di-create, kalo sudah ada di-skip
        //            //kejadian kalo pake tombol Save&Edit, grid control plan-nya tidak refresh (kolom Appointment No masih kosong) sehingga dianggap belum create appointment
        //            if (apptDtb.Rows.Count == 0)
        //            {
        //                var qSchedule = new ParamedicScheduleDate();
        //                if (qSchedule.LoadByPrimaryKey(planItem.ServiceUnitID, planItem.ParamedicID, planItem.ControlPlanDateTime.Year.ToString(), planItem.ControlPlanDateTime.Date))
        ...
        //                            pat.StreetName, pat.District, pat.City, pat.County, pat.State, pat.ZipCode,
        //                            pat.PhoneNo, pat.Email, pat.Ssn, pat.GuarantorID, nosep, AppSession.Parameter.AppointmentStatusOpen,
        //                            pat.MobilePhoneNo, pat.GuarantorCardNo, nosep, 0, AppSession.UserLogin.UserID, AppSession.Parameter.AppointmentTypeControlPlan, null);
        ...
        private void SaveMdsToSepFolder()
        {
            ...
            if (isSaveMdsToSepFolder)
            {
                ...
                printJobParameters.AddNew("p_RegistrationNo", RegistrationNo);
                printJobParameters.AddNew("p_IsForCasemix", IsCallFromCaseMix ? "1" : "0");
                var path = Module.Reports.ReportViewer.SaveFileToGuarantorDocument(AppSession.Parameter.HealthcareInitial, "SLP.01.0089", printJobParameters); //Resume Medis Rawat Inap
            }
        }
        ...
        private DataTable MedicationReceiveDataTable(string registrationNo, string fromRegistrationNo)
        {
            ...
            query.Where(query.IsVoid != true, query.IsContinue == true, query.RegistrationNo.In(regNos));
            ...
            else
            {
                ...
                // Join ke MedicationReceivePrescLog krn RefTransactionNo pada table MedicationReceive hanya menyimpan PrescriptionNo yg pertama (Handono 231101)
                ...
                // Sub Query
                var hp = new MedicationReceivePrescLogQuery("mh");
                var tp = new TransPrescriptionQuery("tp");
                hp.InnerJoin(hp).On(tp.PrescriptionNo == hp.PrescriptionNo);
                hp.Where(hp.MedicationReceiveNo == query.MedicationReceiveNo, tp.IsForTakeItHome == true);
                hp.Select(hp.MedicationReceiveNo);

                query.Where(query.MedicationReceiveNo.In(hp), query.BalanceQty > 0);
            }
            ...
            // Kurangi Qty dengan Home Prescription yg di Void krn tipe Home Prescription langsung menambah ke MedicationReceive tanpa approve
        ...
        public static string PhysicalExaminationHist(string regNo, string fromRegNo)
        {
            var pe = Patient.Last.PhysicalExamination(regNo, fromRegNo, true);
            ...
            if (!string.IsNullOrEmpty(fromRegNo))
                phrl.Where(phrl.Or(phrl.RegistrationNo == regNo, phrl.RegistrationNo == fromRegNo));
            else
                phrl.Where(phrl.RegistrationNo == regNo);
            ...
            // Remark base on WA Chat Olivia Johannes & Silfani RSI (Handono 27 Nov 2023)
            // pemeriksaan TTV kan ada 2 kolom kalo di MDS, yg satu di kolom Pemeriksaan Fisik diambil dari triage
            // ato poli, dan yg satu lagi TTV di kolom Kondisi Pasien Keluar itu diambil dari TTV terakhir
            // jd maksud bu Silfani, yang di kolom Pemeriksaan Fisik, jangan ikutan berubah ikut pemeriksaan TTV terakhir,
            // baik keluar hidup maupun keluar meninggal
            // --- End WA -----
            ...
            // Pemeriksaan Fisik diambil dari triage atau TTV pertama yg nilainya diatas 0 (Handono 27 Nov 2023)
            if (AppParameter.GetParameterValue(AppParameter.ParameterItem.IsShowCurb65ScoreInAssesmentAndMDS) == "Yes")
            {
                //var ndtd = new NursingDiagnosaTransDTQuery("ndtd");
                //phrl.LeftJoin(ndtd).On(phrl.TransactionNo == ndtd.ReferenceToPhrNo);
                //phrl.Where(phrl.Or(
                //    vital.ValueType == "NUM" && phrl.QuestionAnswerNum > 0,
                //    vital.VitalSignInitial == "CURB" && (ndtd.IsDeleted.IsNull() || ndtd.IsDeleted == 0)
                //));
                
                phrl.Where(phrl.Or(
                    vital.ValueType == "NUM" && phrl.QuestionAnswerNum > 0,
                    vital.VitalSignInitial == "CURB")
                );
            }
            else
            {
            ...
            foreach (DataRow row in dtb.Rows)
            {
                ...
                if (!prevId.Equals(row["VitalSignID"]))
                {
                    ...
                    if (string.IsNullOrWhiteSpace(prtId))
                    {
                        ...
                        string answerNumFormatted = string.IsNullOrEmpty(row["QuestionAnswerNum"].ToString()) ? ""
                            : string.Format(numFormat, row["QuestionAnswerNum"]);

                        string answerSuffix = string.IsNullOrEmpty(row["QuestionAnswerSuffix"].ToString()) ? ""
                            : row["QuestionAnswerSuffix"].ToString();

                        string answerText = string.IsNullOrEmpty(answerNumFormatted + answerSuffix)
                            ? row["QuestionAnswerText"].ToString()
                            : $"{answerNumFormatted} {answerSuffix}";

                        rowVts["VitalSign"] = string.Format("{0}: {1}", row["VitalSignName"], answerText);
