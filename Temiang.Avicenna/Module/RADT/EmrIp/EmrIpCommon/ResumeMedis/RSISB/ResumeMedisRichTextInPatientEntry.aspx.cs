...
break;";
        ...
        protected override void OnPopulateEntryControl(ValidateArgs args)
        {
            ...
            if (medRes.LoadByPrimaryKey(RegistrationNo))
            {
                ...
                edtHomeCare.Content = medRes.HomeCare;
                edtHomeEducation.Content = medRes.EducationAtHome;
                edtconsultation.Content = medRes.Consul;
                txtMedicalSupport.Text = medRes.MedicalSupport;
                txtInLocation.Text = medRes.InLocation;
                txtCollectionDateTime.SelectedDate = medRes.CollectionDateTime;
                edtInitialDiagnose.Content = medRes.InitialDiagnose;
        ...
        #region InitialDiagnoseHist
        private string InitialDiagnoseHist(List<string> mergeRegs)
        {
            //Planing Procedure
            // Ambil di Asessmen Therapy
            ...
            var diagnos = new DiagnoseQuery("p");
            query.InnerJoin(diagnos).On(query.DiagnoseID == diagnos.DiagnoseID);

            query.Where(query.RegistrationNo.In(MergeRegistrations), query.SRDiagnoseType.In("DiagnoseType-001", "DiagnoseType-003"));

            query.Select(query.DiagnoseID, query.DiagnosisText);

            var dtbdiagnos = query.LoadDataTable();
            var strb = new StringBuilder();
            strb.AppendLine("<ul>");
            foreach (DataRow row in dtbdiagnos.Rows)
            {
                strb.AppendFormat("<li> {0} - {1}</li>", row["DiagnoseID"], row["DiagnosisText"]);
            }

            strb.AppendLine("</ul>");
            var diagnosHist = strb.ToString();
            return diagnosHist;
        }
        #endregion
        ...
        protected override void OnMenuNewClick()
        {
            ...
            if (!medRes.LoadByPrimaryKey(RegistrationNo))
            {
                edtPhysicalExamination.Content = Temiang.Avicenna.Module.RADT.Emr.ResumeMedisRichTextInPatientEntry.PhysicalExaminationHist(RegistrationNo, FromRegistrationNo);
                ...
                var InitialDiagnose = InitialDiagnoseHist(MergeRegistrations);
                ...
                edtHomeCare.Content = medRes.HomeCare;
                edtHomeEducation.Content = medRes.EducationAtHome;
                edtInitialDiagnose.Content = InitialDiagnose;
        ...
        private void SaveMedicalDischargeSummary()
        {
            ...
            medsum.HomeCare = edtHomeCare.Content;
            medsum.EducationAtHome = edtHomeEducation.Content;
            medsum.Consul = edtconsultation.Content;
            medsum.MedicalSupport = txtMedicalSupport.Text;
            medsum.InLocation = txtInLocation.Text;
            medsum.CollectionDateTime = txtCollectionDateTime.SelectedDate;
            medsum.InitialDiagnose = edtInitialDiagnose.Content;
