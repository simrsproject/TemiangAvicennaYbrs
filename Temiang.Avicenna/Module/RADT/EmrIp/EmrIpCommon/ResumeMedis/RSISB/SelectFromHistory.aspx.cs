...
namespace Temiang.Avicenna.Module.RADT.Emr.ResumeMedis.RSISB
{
    public partial class SelectFromHistory : BasePageDialog
    {
        ...
        private List<SelectedItem> PrescriptionHist(List<string> mergeRegistrations)
        {
            ...
            {
                listItems.Add(new SelectedItem(no, string.Format("{0} ({1} {2} {3} {4})",
                    row["ItemName"], row["TakenQty"], row["SRConsumeMethodName"], row["ConsumeQty"], row["SRConsumeUnit"])));
                no++;
            }
        ...
        private string PrescriptionItemCompound(string prescriptionNo, string sequenceNo, string consumeMethod)
        {
            ...
            (
                ...
                qItem.ItemName,query.TakenQty, query.SRDosageUnit, query.DosageQty,
