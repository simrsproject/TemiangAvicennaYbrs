...
{
    public partial class SafetyCultureIncidentReportsConclusionList : BasePage
    {
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            ProgramID = AppConstant.Program.KEHRS_SafetyCultureIncidentReportsConclusion;
        ...
        private DataTable EmployeeSafetyCultureIncidentReportsOutstandings
        {
            get
            {
                var isEmptyFilter = string.IsNullOrEmpty(txtTransactionNo.Text) && txtReportFromDate.IsEmpty && txtReportToDate.IsEmpty && string.IsNullOrEmpty(txtEmployeeNo.Text) 
                    && txtVerfiedFromDate.IsEmpty && txtVerfiedToDate.IsEmpty && string.IsNullOrEmpty(cboSRIncidentReportStatus.SelectedValue);
                if (!ValidateSearch(isEmptyFilter, "Conclusion & Recommendation List")) return null;
                ...
                query.Select(
                    ...
                    "<'../SafetyCultureIncidentReports/SafetyCultureIncidentReportsDetail.aspx?md=edit&id=' + a.TransactionNo + '&type=con' as EditUrl>"
                    );
                query.Where(query.SRIncidentReportStatus != "04", query.IsVerified == true, query.IsClosed.IsNull());
                ...
                if (!string.IsNullOrEmpty(txtEmployeeNo.Text))
                {
                    ...
                    query.Where(query.Or(qemp.EmployeeNumber == txtEmployeeNo.Text, qemp.EmployeeName.Like(searchTextContain)));
                }
        ...
        private DataTable EmployeeSafetyCultureIncidentReportsVerifications
        {
            get
            {
                var isEmptyFilter = string.IsNullOrEmpty(txtTransactionNo.Text) && txtReportFromDate.IsEmpty && txtReportToDate.IsEmpty && string.IsNullOrEmpty(txtEmployeeNo.Text) && txtVerfiedFromDate.IsEmpty && txtVerfiedToDate.IsEmpty 
                    && string.IsNullOrEmpty(cboSRIncidentReportStatus.SelectedValue) && txtClosedFromDate.IsEmpty && txtClosedToDate.IsEmpty;
                if (!ValidateSearch(isEmptyFilter, "Conclusion & Recommendation List")) return null;
                ...
                query.Select(
                    ...
                    "<'../SafetyCultureIncidentReports/SafetyCultureIncidentReportsDetail.aspx?md=view&id=' + a.TransactionNo + '&type=con' as ViewUrl>"
                    );
                query.Where(query.SRIncidentReportStatus != "04", query.IsVerified == true, query.IsClosed.IsNotNull());
                ...
                if (!txtClosedFromDate.IsEmpty && !txtClosedToDate.IsEmpty)
                {
                    query.Where(query.ClosedDateTime >= txtClosedFromDate.SelectedDate, query.ClosedDateTime < txtClosedToDate.SelectedDate.Value.AddDays(1));
                }
                query.es.Top = AppSession.Parameter.MaxResultRecord;
                return query.LoadDataTable();
            }
