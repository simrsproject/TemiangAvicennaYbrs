...
namespace Temiang.Avicenna.Module.HR.Credential.Process
{
    public partial class CredentialingList : BasePageList
    {
        protected override void OnInit(EventArgs e)
        {
            ...
            UrlPageSearch = "CredentialingSearch.aspx?role=usr";
            UrlPageDetail = "CredentialingDetail.aspx?type=caa&role=usr";
            ProgramID = AppConstant.Program.CredentialCompetencyAssessmentApplication;
        ...
        {
            string script = string.Format(@"gotoAddUrl('{0}', '{1}'); args.set_cancel(true);", "caa", "usr");
            return script;
        }
        ...
        private void RedirectToPageDetail(GridDataItem dataItem, string mode)
        {
            ...
            string url = string.Format("CredentialingDetail.aspx?md={0}&id={1}&type={2}&role={3}", mode, id, "caa", "usr");
        ...
        private DataTable CredentialProcesses
        {
            get
            {
                ...
                else
                {
                    ...
                    query.Where(query.SRProfessionGroup != "01", query.PersonID == AppSession.UserLogin.PersonID.ToInt());
                    query.OrderBy
                        (
