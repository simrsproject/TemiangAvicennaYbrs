...
namespace Temiang.Avicenna.Module.HR.Credential.Process.Status
{
    public partial class StatusList : BasePage
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            {
                case "":
                    ProgramID = AppConstant.Program.CredentialingStatusIndividual;
                    break;
                case "01":
                    ProgramID = AppConstant.Program.CredentialingStatusMedicalCommittee;
                    break;
                case "02":
                    ProgramID = AppConstant.Program.CredentialingStatusNursingCommittee;
                    break;
                case "03":
                    ProgramID = AppConstant.Program.CredentialingStatusKtkl;
                    break;
            }
        ...
        private DataTable CredentialProcessDetails(string transactionNo)
        {
            ...
            var usrapps = new AppUserQuery("apps");
            ...
            query.LeftJoin(usrapps).On(usrapps.UserID == query.LastCredentialApplicationByUserID);
            ...
            query.Select(
                ...
                query.IsCompletelyVerified,
                @"<CONVERT(VARCHAR(11), CASE WHEN a.VerifiedDateTime IS NULL AND a.VerifiedDateTime2 IS NULL THEN GETDATE() WHEN a.VerifiedDateTime IS NULL THEN a.VerifiedDateTime2 WHEN a.VerifiedDateTime2 IS NULL THEN a.VerifiedDateTime ELSE (CASE WHEN a.VerifiedDateTime > a.VerifiedDateTime2 THEN a.VerifiedDateTime ELSE a.VerifiedDateTime2 END) END, 113) AS LastVerifiedDateTimes>",
                @"<CASE WHEN a.VerifiedDateTime IS NULL AND a.VerifiedDateTime2 IS NULL THEN '' WHEN a.VerifiedDateTime IS NULL THEN ver2.UserName WHEN a.VerifiedDateTime2 IS NULL THEN ver1.UserName ELSE (CASE WHEN a.VerifiedDateTime > a.VerifiedDateTime2 THEN ver1.UserName ELSE ver2.UserName END) END AS 'LastLastVerifiedBy'>",

                query.IsCredentialApplication,
                @"<CONVERT(VARCHAR(11), ISNULL(a.LastCredentialApplicationDateTime, GETDATE()), 113) AS LastCredentialApplicationDateTimes>",
                usrapps.UserName.As("LastCredentialApplicationBy"),
