<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/MasterDialog.Master" AutoEventWireup="true" CodeBehind="DocumentHist.aspx.cs" Inherits="Temiang.Avicenna.Module.HR.Credential.Document.DocumentHist" %>
    ...
    <telerik:RadCodeBlock ID="radCodeBlock" runat="server">
        ...
        <script type="text/javascript" language="javascript">
            ...
            function showDetail(mode, pdid) {
                var url = '<%= Helper.UrlRoot() %>/Module/HR/Credential/Document/DocumentUpload.aspx?md=' + mode + '&pdid=' + pdid + '&pid=<%= Request.QueryString["pid"] %>&ccm=rebind&cet=<%=grdDocument.ClientID %>&note=<%= Request.QueryString["note"] %>&type=<%= Request.QueryString["type"] %>&role=<%= Request.QueryString["role"] %>&pg=<%= Request.QueryString["pg"] %>';
            ...
            function ShowFile(id, fileName) {
                ...
                {
                    $.download("DocumentDownload.aspx", "id=" + id);
                }
                else {
                    var url;
                    if (fileName.toLowerCase().includes(".pdf") == true) {
                        url = "<%= Helper.UrlRoot() %>/Module/Reports/PdfUrlViewer.aspx?mode=crddoc&id=" + id;
                    }
                    else {
                        url = "DocumentImageZoomView2.aspx?id=" + id + "&pid=<%= Request.QueryString["pid"] %>";
                    }
                    openWinMaximize(url);
                }
            }
        ...
        <MasterTableView ShowHeader="true" AutoGenerateColumns="False" AllowPaging="false"
            ...
            <Columns>
                ...
                <telerik:GridTemplateColumn UniqueName="File" HeaderText="File">
                    <ItemTemplate>
                        <%# DataBinder.Eval(Container.DataItem, "FileAttachName").ToString() == string.Empty ? string.Empty : string.Format("<a href=\"#\" onclick=\"$.download('DocumentDownload.aspx','id={0}'); return false;\"><img src=\"../../../../../Images/Toolbar/download16.png\" border=\"0\" /></a>",
                                                                            DataBinder.Eval(Container.DataItem, "DocumentID"))%>
                    </ItemTemplate>
