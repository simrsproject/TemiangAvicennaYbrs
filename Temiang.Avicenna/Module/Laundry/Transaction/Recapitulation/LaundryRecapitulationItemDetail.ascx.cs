...
{
    public partial class LaundryRecapitulationItemDetail : BaseUserControl
    {
        ...
        protected override void OnDataBinding(EventArgs e)
        {
            ...
            cboItemID.SelectedValue = (String)DataBinder.Eval(DataItem, LaundryRecapitulationProcessItemMetadata.ColumnNames.ItemID);
            txtQty.Value = Convert.ToDouble(DataBinder.Eval(DataItem, LaundryRecapitulationProcessItemMetadata.ColumnNames.Qty));
            txtQtyRewashing.Value = Convert.ToDouble(DataBinder.Eval(DataItem, LaundryRecapitulationProcessItemMetadata.ColumnNames.QtyRewashing));
            ...
            cboSRItemUnit.SelectedValue = (String)DataBinder.Eval(DataItem, LaundryRecapitulationProcessItemMetadata.ColumnNames.SRItemUnit);
        }
        protected void customValidator_ServerValidate(object source, ServerValidateEventArgs args)
        {
            if (txtQty.Value <= 0 && txtQtyRewashing.Value <= 0)
            {
                args.IsValid = false;
                ((CustomValidator)source).ErrorMessage = string.Format("Quantity / Re-Washing Must Bigger than 0");
                return;
            }
            ...
            if (ViewState["IsNewRecord"].Equals(true))
            {
                ...
                bool isExist = coll.Any(i => (i.ItemID.Equals(cboItemID.SelectedValue)));
        ...
        public Decimal QtyRewashing
        {
            get { return Convert.ToDecimal(txtQtyRewashing.Value); }
        }
