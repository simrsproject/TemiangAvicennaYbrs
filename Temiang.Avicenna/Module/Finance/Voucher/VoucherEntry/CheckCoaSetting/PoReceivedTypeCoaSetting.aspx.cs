...
{
    public partial class PoReceivedTypeCoaSetting : BasePageDialog
    {
        ...
        private DataTable ItemTransactionItems
        {
            get
            {
                ...
                foreach (DataRow row in tbl.Rows)
                {
                    ...
                    toServiceUnit.LoadByPrimaryKey(row["ToServiceUnitID"].ToString());
                    ...
                    switch (row["SRItemType"].ToString())
                    {
                        case "11":
                            {
                                iMed.LoadByPrimaryKey(row["ItemID"].ToString());
                                isInventory = iMed.IsInventoryItem ?? false;
                                break;
                            }
                        case "21":
                            {
                                iNonMed.LoadByPrimaryKey(row["ItemID"].ToString());
                                isInventory = iNonMed.IsInventoryItem ?? false;
                                break;
                            }
                        case "81":
                            {
                                iKitc.LoadByPrimaryKey(row["ItemID"].ToString());
                    ...
                    bool isCost = true;
                    if (!Convert.ToBoolean(row["IsNonMasterOrder"]))
                    {
                        if (isInventory || (Convert.ToBoolean(row["IsAssets"])))
                        {
                            isCost = false;
                            ...
                            if (app.ParameterValue == "Yes")
                            {
                                ...
                                supam.Query.Where(supam.Query.LocationId == row["ToLocationID"].ToString(), supam.Query.ServiceUnitId == /*e.ToServiceUnitID*/ toServiceUnit.ServiceUnitID, supam.Query.ProductAccountId == row["ProductAccountIdNonMaster"].ToString(), supam.Query.SRRegistrationType == (toServiceUnit.SRRegistrationType == string.Empty ? "OPR" : toServiceUnit.SRRegistrationType));
                                if (supam.Query.Load())
                                {
                                    accId = supam.ChartOfAccountIdInventory.HasValue ? supam.ChartOfAccountIdInventory.Value : 0;
                                    sublId = supam.SubledgerIdInventory.HasValue ? supam.SubledgerIdInventory.Value : 0;
                    ...
                    if (isCost)
                    {
                        ...
                        {
                            var pacc = Convert.ToBoolean(row["IsNonMasterOrder"]) ?
                                row["ProductAccountId"].ToString() : row["ProductAccountIdNonMaster"].ToString();
                            //Matrix COA Unit 
                            ServiceUnitProductAccountMapping supam = new ServiceUnitProductAccountMapping();
                            ...
                            supam.Query.Where(supam.Query.ServiceUnitId == row["ToServiceUnitID"].ToString(),
