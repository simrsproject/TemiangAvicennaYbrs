    ...
    public partial class RemunList : BasePage
    {
        public static void PopulateYear(RadComboBox cboYear) {
            ...
            if (tc.LoadByPrimaryKey(BusinessObject.Reference.TransactionCode.RemunerationByIdi))
            {
                ...
                // start from year of efective date
        ...
        {
            var remColl = new ServiceFeeRemunRsucdrCollection();

            if (!string.IsNullOrEmpty(txtRemunNo.Text))
            {
                remColl.Query.Where(remColl.Query.RemunNo == txtRemunNo.Text);
            }
            remColl.Query.OrderBy(remColl.Query.PeriodYear.Descending, remColl.Query.PeriodMonth.Descending, remColl.Query.IsBPJS.Descending);
            remColl.Query.es.Top = AppSession.Parameter.MaxResultRecord;
            remColl.LoadAll();
