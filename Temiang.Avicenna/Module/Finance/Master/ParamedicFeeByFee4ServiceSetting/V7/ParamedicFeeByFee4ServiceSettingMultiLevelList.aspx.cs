        ...
        protected void grdList_PreRender(object sender, EventArgs e)
        {
            ...
            for (int rowIndex = grd.Items.Count - 2; rowIndex >= 0; rowIndex--)
            {
                ...
                if (row["ParamedicStatusName"].Text == previousRow["ParamedicStatusName"].Text &&
                    row["ParamedicName"].Text == previousRow["ParamedicName"].Text &&
                    row["SpecialtyName"].Text == previousRow["SpecialtyName"].Text &&
                    row["RegistrationTypeMergeBillingName"].Text == previousRow["RegistrationTypeMergeBillingName"].Text)
                {
                    row["RegistrationTypeMergeBillingName"].RowSpan = previousRow["RegistrationTypeMergeBillingName"].RowSpan < 2 ?
                        2 : previousRow["RegistrationTypeMergeBillingName"].RowSpan + 1;
                    previousRow["RegistrationTypeMergeBillingName"].Visible = false;
                }
            ...
            foreach (GridDataItem g in grd.MasterTableView.Items)
            {
                foreach (GridColumn c in grd.Columns)
                {
                    ...
                    if (c.UniqueName == "cnew") cCount[grd.Columns.IndexOf(c)] = 1;
                }
            }
        ...
        private DataTable FeeSetting
        {
            get
            {
                ...
                else
                {
                    ...
                    var rTypeMB = new AppStandardReferenceItemQuery("rTypeMB");
                    ...
                    query.LeftJoin(par).On(query.ParamedicID.Equal(par.ParamedicID))
                        ...
                        .LeftJoin(rTypeMB).On(query.SRRegistrationTypeMergeBilling.Equal(rTypeMB.ItemID) && rTypeMB.StandardReferenceID.Equal(AppEnum.StandardReference.RegistrationType))
                        ...
                        (
                            ...
                            rTypeMB.ItemName.As("RegistrationTypeMergeBillingName"),
