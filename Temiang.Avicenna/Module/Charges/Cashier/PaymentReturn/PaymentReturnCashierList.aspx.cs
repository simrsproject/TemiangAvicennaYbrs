...
{
    public partial class PaymentReturnCashierList : BasePage
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            ProgramID = AppConstant.Program.PaymentReturnCashier;
        ...
        private DataTable OutStandings
        {
            get
            {
                ...
                if (!ValidateSearch(isEmptyFilter, "Down Payment Return (Cashier)")) return null;
                ...
                qpy.LeftJoin(qreg).On(qreg.RegistrationNo == qpy.RegistrationNo);
                qpy.InnerJoin(qpat).On(qpat.PatientID == qpy.PatientID);
                qpy.LeftJoin(qpar).On(qpar.ParamedicID == qreg.ParamedicID);
                qpy.LeftJoin(qunit).On(qunit.ServiceUnitID == qreg.ServiceUnitID);
                qpy.LeftJoin(qgrr).On(qgrr.GuarantorID == qreg.GuarantorID);
                ...
                if (!txtPaymentDate1.IsEmpty && !txtPaymentDate2.IsEmpty)
                    qpy.Where(qpy.PaymentDate.Date().Between(txtPaymentDate1.SelectedDate, txtPaymentDate2.SelectedDate));
                ...
                qpy.Where(qpy.TransactionCode == TransactionCode.DownPaymentReturn, qpy.IsApproved == false, qpy.IsVoid == false);
        ...
        private DataTable Payments
        {
            get
            {
                ...
                if (!txtPaymentDate1.IsEmpty && !txtPaymentDate2.IsEmpty)
                    qpy.Where(qpy.ApproveDate.Date().Between(txtPaymentDate1.SelectedDate, txtPaymentDate2.SelectedDate));
                if (cboServiceUnitID.SelectedValue != string.Empty)
                    qpy.Where(qreg.ServiceUnitID == cboServiceUnitID.SelectedValue);
                if (txtRegistrationNo.Text != string.Empty)
                {
                ...
                qpy.Where(qpy.TransactionCode == TransactionCode.DownPaymentReturn, qpy.IsApproved == true, qpy.ApproveByUserID == AppSession.UserLogin.UserID);
