...
{
    public partial class SalesReturnPickList : BasePageDialog
    {
        private DataTable SalesToCustomers
        {
            get
            {
                ...
                query.Where(
                    query.TransactionCode == TransactionCode.Sales,
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    grdDetail.Columns.FindByUniqueName("BatchNumber").Visible = false;
                    grdDetail.Columns.FindByUniqueName("ExpiredDate").Visible = false;
                }
            }
        }
        ...
        {
            ((RadGrid)source).DataSource = SalesToCustomers;
        }

        protected void grdDetail_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            if (ViewState["SalesReturn:Detail" + Request.UserHostName] != null)
                grdDetail.DataSource = ViewState["SalesReturn:Detail" + Request.UserHostName];
        }
        ...
        {
            DataTable dtb = (DataTable)ViewState["SalesReturn:Detail" + Request.UserHostName];
            foreach (GridDataItem dataItem in grdDetail.MasterTableView.Items)
            {
                ...
                ViewState["SalesReturn:Detail" + Request.UserHostName] = dtb;
            }
        }
        private void InitializeDataDetail(string transactionNo)
        {
            ...
            using (new esTransactionScope())
            {
                ...
                query.Select(
                    query.TransactionNo,
                    query.SequenceNo,
                    string.Format("<'{0}' as FromServiceUnitID>", itemTransaction.FromServiceUnitID),
                    string.Format("<'{0}' as FromLocationID>", itemTransaction.FromLocationID),
                    iq.ItemName,
                    query.ItemID,
                    "<p.SRItemUnit>",
                    ...
                    query.BatchNumber, query.ExpiredDate
                    );
        ...
        public override bool OnButtonOkClicked()
        {
            ...
            Session["SalesReturn:ItemSelected" + Request.UserHostName] = ViewState["SalesReturn:Detail" + Request.UserHostName];
            return true;
        }
