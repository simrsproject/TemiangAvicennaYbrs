...
{
    public partial class CrossMatchingDetail : BasePageDetail
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            // Url Search & List
            UrlPageSearch = "##";
            UrlPageList = "CrossMatchingList.aspx";

            ProgramID = AppConstant.Program.BloodBankCrossMatching;

            if (!IsPostBack)
            {
                StandardReference.Initialize(cboSRBloodType, AppEnum.StandardReference.BloodType);
        ...
        {
            //var count = BloodBankTransactionItems.Count(item => item.IsVoid == false);
            //if (count > txtQtyBagRequest.Value)
            //{
            ...
            using (var trans = new esTransactionScope())
            {
                ...
                if (BloodBankTransactionItems.Count > 0)
                {
                    ...
                    foreach (var item in BloodBankTransactionItems)
                    {
                        ...
                        {
                            if (item.IsCrossmatchingPassed == true)
                            {
                                var bagno = bloodBagNos.SingleOrDefault(bn => bn.BagNo == item.BagNo);
                                if (bagno != null)
                                {
                                    bagno.IsCrossMatching = true;
                                    bagno.LastUpdateDateTime = (new DateTime()).NowAtSqlServer();
                                    bagno.LastUpdateByUserID = AppSession.UserLogin.UserID;
                                }
                            }

                            if (item.IsCrossmatchBillProceed == false)
                            {
                                var itemId = AppSession.Parameter.ItemIdBloodCrossMatching;

                                item.IsCrossmatchBillProceed = true;
                                if (!string.IsNullOrEmpty(itemId))
                                {
                                    var i = new Item();
                                    if (i.LoadByPrimaryKey(itemId))
                                    {
                                        Temiang.Avicenna.WebService.BillingChargeService.BillingProcess(entity.RegistrationNo, i.ItemID, "001", 1, "ds", true);
                                    }
                                }
                            }
                        }
        ...
        private BloodBankTransactionItemCollection BloodBankTransactionItems
        {
            get
            {
                if (IsPostBack)
                {
                    object obj = Session["collBloodBankTransactionItemCrossMatching" + Request.UserHostName];
                    if (obj != null)
                    {
                ...
                query.Select
                    (
                        ...
                        usr.UserName.As("refToAppUser_CrossMatchingByUserName"),
                ...
                query.InnerJoin(usr).On(usr.UserID == query.CrossMatchingByUserID);
                ...
                Session["collBloodBankTransactionItemCrossMatching" + Request.UserHostName] = coll;
                return coll;
            }
            set
            {
                Session["collBloodBankTransactionItemCrossMatching" + Request.UserHostName] = value;
            }
        }
        ...
        private void RefreshCommandItem(AppEnum.DataMode newVal)
        {
            ...
            grdItem.Columns[1].Visible = !isVisible && AppSession.Parameter.IsShowCrossMatchingPrintLabel;
        ...
        protected void grdItem_UpdateCommand(object source, GridCommandEventArgs e)
        {
            ...
            if (entity != null && entity.IsVoid == false && entity.SRBloodBagStatus == "1")
        ...
        protected void grdItem_DeleteCommand(object source, GridCommandEventArgs e)
        {
            ...
            BloodBankTransactionItem entity = FindItem(bagNo);
            if (entity != null && entity.SRBloodBagStatus == "1")
                entity.IsVoid = true;
        ...
        protected void grdItem_ItemCommand(object source, GridCommandEventArgs e)
        {
            if (e.CommandName == "PrintLabel")
            {
                ...
                parPaymentNo.Name = "p_TransactionNo";
                parPaymentNo.ValueString = txtTransactionNo.Text;

                var parUserName = jobParameters.AddNew();
                parUserName.Name = "p_BagNo";
                parUserName.ValueString = e.CommandArgument.ToString();

                AppSession.PrintJobParameters = jobParameters;
                AppSession.PrintJobReportID = AppConstant.Report.BloodBankCrossMatchingLabel;
        ...
        {
            var userControl = (CrossMatchingItemDetail)e.Item.FindControl(GridEditFormItem.EditFormUserControlID);
            if (userControl != null)
            {
                ...
                entity.SRBloodBagStatus = "1";
                entity.BloodBagStatusName = "Cross Matching";
                ...
                entity.CrossMatchingByUserID = userControl.CrossMatchingByUserID;
                entity.CrossMatchingByUserName = userControl.CrossMatchingByUserName;
                entity.SRBloodSource = userControl.SrBloodSource;
                entity.SRBloodSourceFrom = userControl.SrBloodSourceFrom;
                entity.CrossmatchStartDateTime = userControl.CrossmatchStartDateTime;
                entity.CrossmatchEndDateTime = userControl.CrossmatchEndDateTime;
                entity.IsCrossmatchingPassed = userControl.IsCrossmatchingPassed;
                entity.IsVoid = userControl.IsVoid;
                entity.IsCrossmatchBillProceed = userControl.IsCrossmatchBillProceed;
