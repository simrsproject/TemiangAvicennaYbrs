...
{
    public partial class LiquidFoodDietSettingItemDetail : BaseUserControl
    {
        ...
        protected override void OnDataBinding(EventArgs e)
        {
            ...
            cboTime.SelectedValue = (String)DataBinder.Eval(DataItem, LiquidFoodDietTimeMetadata.ColumnNames.Time);

            var foodId = Convert.ToString(DataBinder.Eval(DataItem, LiquidFoodDietTimeMetadata.ColumnNames.FoodID));
            ...
            foodId = Convert.ToString(DataBinder.Eval(DataItem, LiquidFoodDietTimeMetadata.ColumnNames.ChildrenFoodID));
            if (!string.IsNullOrEmpty(foodId))
            {
        ...
        protected void customValidator_ServerValidate(object source, ServerValidateEventArgs args)
        {
            ...
            {
                var coll = (LiquidFoodDietTimeCollection)Session["collLiquidFoodDietTime" + Request.UserHostName];

                string time = cboTime.SelectedValue;

                bool isExist = false;
                foreach (LiquidFoodDietTime item in coll)
                {
