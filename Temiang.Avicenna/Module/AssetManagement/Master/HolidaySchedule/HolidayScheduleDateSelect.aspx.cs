...
{
    public partial class HolidayScheduleDateSelect : BasePageDialog
    {
        ...
        public override bool OnButtonOkClicked()
        {
            ...
            var dtb = (DataTable)Session["dtbAssetHolidaySchedule"];
            ...
            foreach (RadDate date in selectedDates)
            {
                ...
                {
                    row = dtb.NewRow();
                    row["HolidayDate"] = date.Date;
                    dtb.Rows.Add(row);
                }
                else if (chkIsVoid.Checked)
                    row.Delete();
            }
            return true;
        }
