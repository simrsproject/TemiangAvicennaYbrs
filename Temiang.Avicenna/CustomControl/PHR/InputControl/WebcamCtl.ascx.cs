...
{
    public partial class WebcamCtl : BasePhrCtl
    {
        ...
        protected override void OnSetEntityValue(ValidateArgs args, Patient pat, Registration reg, PatientHealthRecord phr, PatientHealthRecordLine phrLine, string lastRegistrationNo)
        {
            ...
            {
                var imageVal = hdnImage.Value;
                if (imageVal.Contains("base64"))
                    imageVal = imageVal.Substring(22); // buang -> data:image/Png;base64,

                var imgHelper = new ImageHelper();
                var resized = imgHelper.ResizeImage(imgHelper.ToImage(imageVal), new Size(332, 185));
