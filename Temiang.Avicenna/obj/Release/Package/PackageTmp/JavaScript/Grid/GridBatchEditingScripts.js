...
Telerik.Web.UI.GridBatchEditing.prototype={initialize:function(){var s=this;
var p=s._owner.get_element();
var n=s._owner._batchEditingOpenForEditEvents;
var o;
s._batchEditingPanel=$telerik.findElement(p,"BatchEditingPanel");
s._batchEditingDelegates=[];
var r=false;
for(var q in n){if(n[q].toLowerCase){o=n[q].toLowerCase();
if(o!=="none"){if($telerik.isIE&&o=="mousedown"){o="click";
}if(o=="click"){if(r){continue;
}r=true;
}s._batchEditingDelegates.push([o,$telerik.addMobileHandler(s,p,o,s._openEditFromEvent)]);
}}}s._onMouseDownDelegate=$telerik.addMobileHandler(s,document,"mousedown",s._mouseDown,null,true);
s._onMouseUpDelegate=$telerik.addMobileHandler(s,document,"mouseup",s._mouseUp,null,true);
s._owner.add_command(s._onGridCommand);
...
u._owner.raise_rowDeleted(new Telerik.Web.UI.GridDataItemEventArgs(q));
return true;
},_deleteAlreadyInsertedRowElements:function(n){var o=this;
a(n.get_element()).find(".rgRow,.rgAltRow").each(function(){if(this.id.indexOf("-")!==-1){o._moveControls(n,this);
n._deleteRow(this);
}});
...
q.addClass("t-button t-button-icontext rgBatchUndoDeleteButton").click(a.proxy(t._undoDelete,t)).css({left:q.scrollLeft()+7});
if(s.get_owner().get_renderMode()!==1){q.html('<span class="t-font-icon rgIcon rgBatchUndoDeleteIcon"></span><span class="t-text rgButtonText">Undo</span>');
...
if(!s||n===w){s=r;
}}}else{if(x._currentlyEditedRow){n=x._currentlyEditedRow.children[t];
q=x._getCellDataToOpenEdit(n);
if(q){p=x._getColumn(q.tableView,n);
if(p.Display&&Sys.UI.DomElement.containsCssClass(n,f)){if(!x._validate()){return true;
}x._updateCellValue(q);
...
var o;
if(t.tagName.toLowerCase()==="td"||(u._currentlyEditedRow&&n.parentNode.id!==u._currentlyEditedRow.id)){if(u._currentlyEditedRow&&n.parentNode.id!==u._currentlyEditedRow.id){t=u._currentlyEditedRow.cells[n.cellIndex];
}o=u._getCellDataToOpenEdit(t);
if(o&&$telerik.getElementByClassName(o.cell,e).style.display===""){if(!u._updateCellValue(o)){return false;
...
},_findItemByText:function(n,r){var p=n.get_items();
var q=p.get_count();
for(var o=0;
o<q;
o++){if(p.getItem(o).get_text().trim()==r){return p.getItem(o);
}}return null;
...
}else{if(F.RadComboBox&&F.RadComboBox.isInstanceOfType(p)||F.RadDropDownList&&F.RadDropDownList.isInstanceOfType(p)){if(E){var C=G._findItemByText(p,H);
...
return true;
},_updateClientDataSourceChanges:function(p){var r=this;
var o=$find(p.get_owner()._clientDataSourceID);
var n=o.get_dataSourceObject().options.autoSync;
var q=p.get_owner().get_batchEditingManager()._changes[p.get_id()];
o.get_dataSourceObject().options.autoSync=false;
p._owner._clientSideBinding._supressChange=true;
r._loopChanges(p,function(t,s){switch(t){case"add":o.insert(0,r._prepareDataItem(p,s.dataItem));
break;
case"update":o.update(r._prepareDataItem(p,s.dataItem),s.dataKey);
break;
case"remove":o.remove(s.dataKey);
break;
}},true);
p._owner._clientSideBinding._supressChange=false;
r._removeChanges();
r._disposeOverlayDivs(p);
r._deleteAlreadyInsertedRowElements(p);
if(q&&q.alreadyInsertedIndexes){p.get_owner().get_batchEditingManager()._changes[p.get_id()].alreadyInsertedIndexes={};
}o.sync();
...
if(B.indexOf("-")!==-1&&t===null){delete F[B];
