<%@ Page Language="C#" MasterPageFile="~/MasterPage/MasterDialog.Master" AutoEventWireup="true"
    CodeBehind="RegistrationVisitHist.aspx.cs" Inherits="Temiang.Avicenna.Module.RADT.PharmaceuticalCare.RegistrationVisitHist" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <telerik:RadCodeBlock ID="radCodeBlock" runat="server">
        ...
        <script type="text/javascript" language="javascript">
            ...
            function entryVisit(mod, visitno) {
                var grdid = "";
                grdid = "<%=grdList.ClientID %>";
                var url = "RegistrationVisitEntry.aspx?mod=" +
                    mod +
                    '&patid=<%= PatientID %>&regno=<%=RegistrationNo %>&vn=' +
                    visitno +
                    '&ccm=rebind&cet=' +
                    grdid;
    ...
    <telerik:RadGrid ID="grdList" runat="server" OnNeedDataSource="grdList_NeedDataSource" OnDeleteCommand="grdList_DeleteCommand" OnItemDataBound="grdList_ItemDataBound"
        ...
        AllowSorting="true">
        <MasterTableView DataKeyNames="VisitNo,IsDeleted">
            <Columns>
                <telerik:GridTemplateColumn UniqueName="TemplateItemName3" HeaderStyle-Width="30px">
                    <ItemTemplate>
                        <%# (!AppSession.UserLogin.UserID.Equals(DataBinder.Eval(Container.DataItem, "CreatedByUserID")) || (DataBinder.Eval(Container.DataItem, "IsDeleted") != DBNull.Value && DataBinder.Eval(Container.DataItem, "IsDeleted").Equals(true))  ? string.Format("<img src=\"{0}/Images/Toolbar/edit16_d.png\" />",Helper.UrlRoot()) : string.Format("<a href=\"#\" onclick=\"javascript:entryVisit('edit', '{0}'); return false;\"><img src=\"{1}/Images/Toolbar/edit16.png\"  alt=\"New\" /></a>",DataBinder.Eval(Container.DataItem, "VisitNo"),Helper.UrlRoot()))%>
                    </ItemTemplate>
                </telerik:GridTemplateColumn>
                <telerik:GridBoundColumn DataField="VisitNo" HeaderText="No" UniqueName="VisitNo" SortExpression="VisitNo">
                    <HeaderStyle HorizontalAlign="Center" Width="50px" />
                    <ItemStyle HorizontalAlign="Center" />
                </telerik:GridBoundColumn>
                <telerik:GridTemplateColumn UniqueName="VisitDateTime" HeaderText="Visit Time">
                    <ItemTemplate>
                        <%# Convert.ToDateTime(DataBinder.Eval(Container.DataItem, "VisitDateTime")).ToString(AppConstant.DisplayFormat.DateShortMonth) %><br />
                        <%# Convert.ToDateTime(DataBinder.Eval(Container.DataItem, "VisitDateTime")).ToString(AppConstant.DisplayFormat.HourMin) %>
                    </ItemTemplate>
                    <HeaderStyle HorizontalAlign="Center" Width="90px" />
                    <ItemStyle HorizontalAlign="Center" />
                </telerik:GridTemplateColumn>
                <telerik:GridTemplateColumn UniqueName="VisitNotes" HeaderText="Visit Notes">
                    <ItemTemplate>
                        <%# DataBinder.Eval(Container.DataItem, "VisitNotes").ToString().Replace(Environment.NewLine,"<br/>") %>
                    </ItemTemplate>
                    <HeaderStyle HorizontalAlign="Left"/>
                    <ItemStyle HorizontalAlign="Left" />
                </telerik:GridTemplateColumn>
                <telerik:GridBoundColumn DataField="CreatedByUserName" HeaderText="By" UniqueName="CreatedByUserName">
                    <HeaderStyle HorizontalAlign="Center" Width="150px" />
                    <ItemStyle HorizontalAlign="Left" />
                </telerik:GridBoundColumn>
                <telerik:GridBoundColumn DataField="LicenseNo" HeaderText="License No" UniqueName="LicenseNo">
