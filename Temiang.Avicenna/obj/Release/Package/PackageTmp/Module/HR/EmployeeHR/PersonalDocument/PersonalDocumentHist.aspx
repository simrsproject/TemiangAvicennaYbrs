<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/MasterDialog.Master" AutoEventWireup="true" CodeBehind="PersonalDocumentHist.aspx.cs" Inherits="Temiang.Avicenna.Module.HR.EmployeeHR.PersonalDocumentHist" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <telerik:RadCodeBlock ID="radCodeBlock" runat="server">
        ...
        <script type="text/javascript" language="javascript">
            function tbarMain_OnClientButtonClicking(sender, args) {
                ...
                switch (val) {
                    ...
                    case "newupload":
                        ...
                        break;
                    case "batchupload":
                        var url = '<%= Helper.UrlRoot() %>/Module/HR/EmployeeHR/PersonalDocument/PersonalDocumentBatchUpload.aspx?pid=<%= Request.QueryString["pid"] %>&ccm=rebind&cet=<%=grdDocument.ClientID %>&pageId=<%= Request.QueryString["pageId"] %>';
                        openWinMaximize(url);
                        break;
                    case "newwebcam":
                        var url = '<%= Helper.UrlRoot() %>/Module/HR/EmployeeHR/PersonalDocument/PersonalDocumentWebCam.aspx?mod=new&pid=<%= Request.QueryString["pid"] %>&ccm=rebind&cet=<%=grdDocument.ClientID %>&dc=<%= Request.QueryString["dc"] %>&rid=<%= Request.QueryString["rid"] %>&note=<%= Request.QueryString["note"] %>&pageId=<%= Request.QueryString["pageId"] %>';
            ...
            function showDetail(mode, pdid) {
                var url = '<%= Helper.UrlRoot() %>/Module/HR/EmployeeHR/PersonalDocument/PersonalDocumentUpload.aspx?md=' + mode + '&pdid=' + pdid +'&pid=<%= Request.QueryString["pid"] %>&ccm=rebind&cet=<%=grdDocument.ClientID %>&dc=<%= Request.QueryString["dc"] %>&rid=<%= Request.QueryString["rid"] %>&note=<%= Request.QueryString["note"] %>&pageId=<%= Request.QueryString["pageId"] %>';
            ...
            function ShowFile(id, fileName) {
                ...
                {
                    $.download("PersonalDocumentDownload.aspx", "id=" + id);
                }
                else {
                    var url;
                    if (fileName.toLowerCase().includes(".pdf") == true) {
                        url = "<%= Helper.UrlRoot() %>/Module/Reports/PdfUrlViewer.aspx?mode=empdoc&id=" + id;
                    }
                    else {
                        url = "PersonalDocumentImageZoomView2.aspx?id=" + id +"&pid=<%= Request.QueryString["pid"] %>";
                    }
                    openWinMaximize(url);
                }
            }
    ...
    <telerik:RadGrid runat="server" ID="grdDocument" OnNeedDataSource="grdDocument_NeedDataSource"
        ...
        <MasterTableView ShowHeader="true" AutoGenerateColumns="False" AllowPaging="false"
            DataKeyNames="PersonalDocumentID">
                ...
                <telerik:GridTemplateColumn UniqueName="colMenu" HeaderText="" HeaderStyle-Width="30px">
                    <ItemTemplate>
                        <%# string.Format("<a href=\"#\" onclick=\"showDetail('view', '{0}')\"><img src=\"../../../../Images/Toolbar/views16.png\" border=\"0\" /></a>",
                                                                            DataBinder.Eval(Container.DataItem, "PersonalDocumentID"))%>
                    </ItemTemplate>
                </telerik:GridTemplateColumn>
                <telerik:GridTemplateColumn UniqueName="File" HeaderText="File">
                    <ItemTemplate>
                        <%# DataBinder.Eval(Container.DataItem, "FileAttachName").ToString() == string.Empty ? string.Empty : string.Format("<a href=\"#\" onclick=\"$.download('PersonalDocumentDownload.aspx','id={0}'); return false;\"><img src=\"../../../../Images/Toolbar/download16.png\" border=\"0\" /></a>",
                                                                            DataBinder.Eval(Container.DataItem, "PersonalDocumentID"))%>
                    </ItemTemplate>
                    ...
                    <ItemTemplate>
                        <div style="width: 130px; text-align: center;">
                            <asp:LinkButton ID="lbtnDocumentImage" runat="server" ToolTip="View"
                                OnClientClick='<%#string.Format("javascript:ShowFile(\"{0}\",\"{1}\");return false;",DataBinder.Eval(Container.DataItem, "PersonalDocumentID"), Eval("FileAttachName"))%>'>
