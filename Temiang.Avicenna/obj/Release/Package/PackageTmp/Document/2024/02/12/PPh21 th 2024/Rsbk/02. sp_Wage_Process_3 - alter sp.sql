...
AS    
 ...
 BEGIN    
  DECLARE @Gapok                     INT,    
          ...
          @Pph21ThrDTP               INT = 60,    
          @Thr                       INT,    
          @ThrX                      INT = 63, -- faktor pengali thr
          @Overtime                  INT,    
          @GajiNettoSetahun          INT = 24, --Penghasilan Netto Setaun          
          @GajiNettoSetahunThr       INT = 25, --Penghasilan Netto Setaun + THR          
          @GajiBruttoSetahunDtp      INT = 27, --Penghasilan Brutto Setaun DTP    
          @FaktorRuleDisplay         VARCHAR(20),
          
          @GapokBlnLalu INT = 62,
          @Insentif INT = 77,
          --@CattHutangInsentif INT = 83,
          @HutangInsentif INT = 87,
          @InsentifDibayar INT = 86,
          @fxPph21 INT = 95
  ...
  BEGIN    
      ...
      BEGIN
			...
			INNER JOIN (SELECT p.PersonID, 
							ISNULL((SELECT TOP 1 wp.Points FROM EmployeeWageStructureAndScale AS wp WHERE wp.PersonID = p.PersonID AND wp.ValidFrom <= @date ORDER BY wp.ValidFrom DESC), 0) AS Points
     ...
     ' THEN ( B.FaktorRule * D.NominalAmount )' +    
          ...
          BEGIN    
              ...
              FROM   ##Temp_WageTransactionItem A WITH (NOLOCK)    
                     ...
                     INNER JOIN  ##Temp_WageTransactionItem C WITH (NOLOCK) ON C.WageTransactionID = B.WageTransactionID AND C.SalaryComponentID = @Thr
  ...
  INNER JOIN ##Temp_WageTransactionItem D WITH (NOLOCK)    
         ...
         CASE     
          ...
          BEGIN    
              ...
              FROM   (    
                         SELECT A.PersonID,    
                                ...
                                (B.CurrencyAmount / F.CurrencyAmount) AS NominalAmount    
                         FROM   ##Temp_WageTransaction A WITH (NOLOCK)    
                                INNER JOIN ##Temp_WageTransactionItem B WITH (NOLOCK)    
                                     ON  A.WageTransactionID = B.WageTransactionID    
                                     AND B.SalaryComponentID = @pph21thn  
                                INNER JOIN  ##Temp_WageTransactionItem F WITH (NOLOCK) 
									  ON F.WageTransactionID = A.WageTransactionID 
									  AND F.SalaryComponentID = @fxPph21   
                     ) X    
                     INNER JOIN ##Temp_WageTransaction D WITH (NOLOCK)    
                          ON  X.PersonID = D.PersonID    
                     INNER JOIN ##Temp_WageTransactionItem E WITH (NOLOCK)    
          ...
          BEGIN    
              UPDATE ##Temp_WageTransactionItem    
              SET    NominalAmount = 0    
              FROM   ##Temp_WageTransactionItem X WITH (NOLOCK)    
                     INNER JOIN ##Temp_WageTransaction D WITH (NOLOCK)    
		  ...
		  BEGIN    
			...
			FROM   ##Temp_WageTransactionItem A WITH (NOLOCK)    
                INNER JOIN ##Temp_WageTransaction B WITH (NOLOCK)    
                    ON  B.WageTransactionID = A.WageTransactionID    
                    AND A.SalaryComponentID = @SalaryComponentID 
                INNER JOIN ##Temp_WageTransactionItem C WITH (NOLOCK) ON C.WageTransactionID = B.WageTransactionID AND C.SalaryComponentID = @Gapok  
			WHERE A.NominalAmount = 0
		  END 
		  
		  -----------------------------------------------------------------------------------    
          -- 12> Update Hutang insentif = 0 jika lebih besar dari nol
          ----------------------------------------------------------------------------------- 
		  IF (@SalaryComponentID = @HutangInsentif)
		  BEGIN
		  	UPDATE ##Temp_WageTransactionItem    
			SET NominalAmount = 0,  
				CurrencyAmount = 0
		  ...
		  -----------------------------------------------------------------------------------    
          -- 13> Update insentif yg dibayar = 0 jika kurang dari nol
          ----------------------------------------------------------------------------------- 
		  IF (@SalaryComponentID = @InsentifDibayar)
		  BEGIN
		  	UPDATE ##Temp_WageTransactionItem    
			SET NominalAmount = 0,  
				CurrencyAmount = 0
