 ...
 * Time: 10/10/2023 9:25:25 PM
 ************************************************************/

CREATE PROCEDURE sp_ApptQueueingGetLastCalledBeeingServed(@p_Date DATETIME, @p_Prefix VARCHAR(6))
AS
	--DECLARE @p_Date DATETIME = '2023-09-14',
	--@p_Prefix VARCHAR(6) = 'AN'
	
	;WITH LastQueue AS (
	    SELECT aq.SRQueueingLocation,
	           aq.SRQueueingGroup,
	           aq.SRQueueingType,
	           aq.ProcessDateTime,
	           aq.ServiceUnitID,
	           ...
	           aq.CounterCode,
	           ROW_NUMBER() OVER(
	               PARTITION BY SRQueueingGroup ORDER BY SRQueueingLocation DESC
	           )                    AS rn
	    FROM   AppointmentQueueing  AS aq WITH (NOLOCK)
	    WHERE  aq.QueueingDate = @p_Date
	           AND SUBSTRING(aq.FormattedNo, 1, CHARINDEX('-', aq.FormattedNo, 1) - 1) = @p_Prefix
