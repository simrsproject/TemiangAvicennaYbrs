...
AS    
 ...
 BEGIN    
  DECLARE @Gapok                     INT,    
          ...
          @Pph21ThrDTP               INT = 90,    
          @Thr                       INT,    
          @ThrX                      INT = 149, -- faktor pengali thr
          @Overtime                  INT,    
          @GajiNettoSetahun          INT = 74, --Penghasilan Netto Setaun          
          @GajiNettoSetahunThr       INT = 75, --Penghasilan Netto Setaun + THR          
          @GajiBruttoSetahunDtp      INT = 77, --Penghasilan Brutto Setaun DTP    
          @FaktorRuleDisplay         VARCHAR(20),
          
          @GapokBlnLalu INT = 130,
          @GapokBJPSBlnLalu INT  = 145
 ...
 BEGIN    
  ...
  BEGIN    
      ...
      -----------------------------------------------------------------------------------             
      IF (    
             @SalaryComponentID = @Gapok    
             AND @IsUsingStandardSalary = 'Yes'    
         )    
      BEGIN    
          UPDATE ##Temp_WageTransactionItem    
          SET    Qty               = 1,    
                 NominalAmount     = (A.AmountSalary * ISNULL(D.PercentageAmount, 0) / 100),
                 CurrencyAmount = (A.AmountSalary * ISNULL(D.PercentageAmount, 0) / 100)  
          FROM   (    
                     SELECT ss.PositionGradeID,    
                            ssf.GradeServiceYear,    
                            ssf.AmountSalary    
                     FROM   StandardSalary AS ss        WITH(NOLOCK)
                            INNER JOIN StandardSalaryFaktor AS ssf        WITH(NOLOCK)
                                 ON  ssf.StandardSalaryID = ss.StandardSalaryID    
                     WHERE  @date BETWEEN ss.ValidFrom AND ss.ValidTo    
                 ) A    
                 INNER JOIN ##Temp_WageTransaction B        WITH(NOLOCK)
                      ON  A.PositionGradeID = B.PositionGradeID    
                      AND CAST(a.GradeServiceYear AS INT) = CAST(B.GradeYear AS INT)    
                 INNER JOIN ##Temp_WageTransactionItem C        WITH(NOLOCK)
                      ON  B.WageTransactionID = C.WageTransactionID    
                      AND C.SalaryComponentID = @Gapok    
                 INNER JOIN SalaryComponentRuleDefinition AS D        WITH(NOLOCK)
                      ON  D.SalaryComponentID = C.SalaryComponentID    
                      AND D.SREmploymentType = B.SREmploymentType    
      END     
      ...
      BEGIN    
          ...
          BEGIN    
              ...
              FROM   ##Temp_WageTransactionItem A        WITH(NOLOCK)
                     ...
                     INNER JOIN ##Temp_WageTransaction C        WITH(NOLOCK)
      ...
      BEGIN    
          ...
          FOR    
              ...
              ON sc.SalaryComponentID = x.SalaryRuleComponentID
     ...
     ' THEN ( B.FaktorRule * D.NominalAmount )' +    
              ...
              BEGIN    
                  ...
                  SET @SQLScrip = @SQLScrip +    
                      ...
                      +    
                      ' AS Qty FROM MontlyAttedance     WITH(NOLOCK) WHERE PayrollPeriodID= '     
          ...
          BEGIN    
           ...
           SET    NominalAmount = CASE     
                                  ...
                                  END    
           FROM   ##Temp_WageTransaction A        WITH(NOLOCK)
                  INNER JOIN ##Temp_WageTransactionItem B        WITH(NOLOCK)
          ...
          BEGIN    
              ...
              SET    A.NominalAmount = CASE     
                                          ...
                                          ELSE (    
                                                  CASE WHEN  CAST ((LEFT((RIGHT([dbo].[fnCalculateAgeTHR](vet.JoinDate,@dateThr),4)),2)) AS NUMERIC(8,0)) > 14  THEN 
	                                              DATEDIFF(MONTH, vet.JoinDate, @date) + 1 ELSE DATEDIFF(MONTH, vet.JoinDate, @date) END / 12.00    
                                               )  
                                     END,    
              ...
              FROM   ##Temp_WageTransactionItem A        WITH(NOLOCK)
                     ...
                     INNER JOIN  ##Temp_WageTransactionItem C    WITH(NOLOCK)
                      ON C.WageTransactionID = B.WageTransactionID AND C.SalaryComponentID = @Thr
                     INNER JOIN Vw_EmployeeTable AS vet    
                          ON  vet.PersonID = B.PersonID    
			
          END   
          
          --THR NOL-kan amount yg tidak termasuk dalam proses periodic salary         
              ...
              FROM   ##Temp_WageTransaction A        WITH(NOLOCK)
                     ...
                     INNER JOIN (    
                              SELECT C.PersonID,    
                                     ...
                                     D.NominalAmount,    
                                     D.CurrencyAmount    
                              FROM   ##Temp_WageTransaction C        WITH(NOLOCK)
                                     INNER JOIN ##Temp_WageTransactionItem D       WITH(NOLOCK) 
  ...
  INNER JOIN ##Temp_WageTransactionItem D       WITH(NOLOCK) 
          ...
          BEGIN    
              ...
              FROM   (    
                     ...
                     INNER JOIN ##Temp_WageTransaction D        WITH(NOLOCK)
                          ON  X.PersonID = D.PersonID    
                     INNER JOIN ##Temp_WageTransactionItem E        WITH(NOLOCK)
         ...
         CASE     
          ...
          BEGIN    
              ...
              WHERE  D.PersonID NOT IN (SELECT eps.PersonID    
                                        FROM   EmployeePeriodicSalary AS eps    
		  ...
		  BEGIN    
			...
			FROM   ##Temp_WageTransactionItem A        WITH(NOLOCK)
                INNER JOIN ##Temp_WageTransaction B        WITH(NOLOCK)
                    ...
                    AND A.SalaryComponentID = @SalaryComponentID 
                INNER JOIN ##Temp_WageTransactionItem C     WITH(NOLOCK)
                ON C.WageTransactionID = B.WageTransactionID AND C.SalaryComponentID = @Gapok  
			WHERE A.NominalAmount = 0
		  END 
		      
          -----------------------------------------------------------------------------------    
          -- 12> Update Dasar Perhitungan BPJS Kesehatan (Bulan Lalu) = Dasar Perhitungan BPJS Kesehatan skr jika nol
          -----------------------------------------------------------------------------------  
		  IF (@SalaryComponentID = @GapokBJPSBlnLalu)
		  BEGIN    
			UPDATE A    
			SET A.NominalAmount = C.NominalAmount,  
				A.CurrencyAmount = C.CurrencyAmount
