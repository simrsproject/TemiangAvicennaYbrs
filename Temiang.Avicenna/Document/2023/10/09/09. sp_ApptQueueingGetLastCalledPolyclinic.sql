 ...
 * Time: 10/9/2023 8:01:27 PM
 ************************************************************/

CREATE PROCEDURE sp_ApptQueueingGetLastCalledPolyclinic(@p_Date DATETIME)
AS
	...
	WITH LastQueue AS (
	    SELECT aq.Id,
	           ...
	           aq.CounterCode,
	           ROW_NUMBER() OVER(
	               PARTITION BY SRQueueingLocation ORDER BY ProcessDateTime DESC
	           )                    AS rn
	    FROM   AppointmentQueueing  AS aq WITH (NOLOCK)
	    WHERE  aq.QueueingDate = @p_Date
	           AND aq.SRQueueingGroup = '02'
